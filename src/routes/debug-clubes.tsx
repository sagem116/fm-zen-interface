...
function normName(s: string) {
  return s
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/\b(fc|cf|sc|ac|afc|cd|sd|as|ss|us|ud|ss|club|fk|bk|de|do|of|the)\b/g, "")
    .replace(/[^a-z0-9]/g, " ")
    .split(/\s+/)
    .filter(Boolean)
    .sort()
...
function DebugClubes() {
  ...
  // Hydrate localStorage mirror from Supabase so this page reflects
  // exactly what's in `club_reputation_season`.
  useEffect(() => {
    void hydrateClubReputationsFromDb();
  }, []);
  ...
  const derived = useMemo(() => {
    ...
    const latestYear = psData?.players?.length
      ? Math.max(...psData.players.map((p) => p.season_year))
      : players.length
        ? Math.max(...players.map((p) => p.season_year))
        : 0;
    ...
    for (const s of standings) {
      if (s.club_name && s.competition && !clubCompLatest.has(s.club_name))
        clubCompLatest.set(s.club_name, s.competition);
    }

    // Sections
    const clubsWithoutPlayers = allClubs
      .filter((c) => (playersPerClubLatest.get(c) ?? 0) === 0)
      .sort();
    ...
    const clubsWithoutReputation = allClubs
      .filter((c) => reputationFor(c, aliases, reps) == null)
      .sort();
    const clubsWithoutCompetition = allClubs.filter((c) => !clubCompLatest.has(c)).sort();
    const clubsWithoutHistory = allClubs
      .filter((c) => !clubsWithAnyPlayers.has(c) && !clubsInStandings.has(c))
      .sort();
    ...
    return {
      ...
      clubsWithoutPlayers,
      clubsWithoutCountry,
      clubsWithoutContinent,
      clubsWithoutReputation,
      clubsWithoutCompetition,
      clubsWithoutHistory,
      duplicates,
      similar,
      noCountryRows,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Diagnóstico completo dos clubes: dados em falta, duplicados e histórico.
          </p>
        </div>
        <div className="flex gap-2">
          <Button asChild size="sm" variant="outline">
            <Link to="/debug-reputacao-clubes" search={{ tab: undefined }}>
              <ExternalLink className="size-3.5" /> Reputação de Clubes
            </Link>
          </Button>
          <Button asChild size="sm" variant="outline">
            <Link to="/debug-mapeamento-clubes" search={{ tab: undefined }}>
              <ExternalLink className="size-3.5" /> Mapeamento de Clubes
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4 lg:grid-cols-8">
        ...
        <Stat
          label="Sem jogadores"
          value={derived.clubsWithoutPlayers.length}
          tone={derived.clubsWithoutPlayers.length ? "warn" : "ok"}
        />
        <Stat
          label="Sem país"
          value={derived.clubsWithoutCountry.length}
          tone={derived.clubsWithoutCountry.length ? "warn" : "ok"}
        />
        <Stat
          label="Sem continente"
          value={derived.clubsWithoutContinent.length}
          tone={derived.clubsWithoutContinent.length ? "warn" : "ok"}
        />
        <Stat
          label="Sem reputação"
          value={derived.clubsWithoutReputation.length}
          tone={derived.clubsWithoutReputation.length ? "warn" : "ok"}
        />
        <Stat
          label="Sem competição"
          value={derived.clubsWithoutCompetition.length}
          tone={derived.clubsWithoutCompetition.length ? "warn" : "ok"}
        />
        <Stat
          label="Duplicados"
          value={derived.duplicates.length}
          tone={derived.duplicates.length ? "warn" : "ok"}
        />
        <Stat
          label="Sem histórico"
          value={derived.clubsWithoutHistory.length}
          tone={derived.clubsWithoutHistory.length ? "warn" : "ok"}
        />
      </div>

      <ClubListCard
        title="Clubes sem jogadores (última época)"
        clubs={derived.clubsWithoutPlayers}
      />
      ...
      <ClubListCard
        title="Clubes sem competição associada"
        clubs={derived.clubsWithoutCompetition}
      />
      <ClubListCard
        title="Clubes sem histórico (nem em standings nem em player_stats)"
        clubs={derived.clubsWithoutHistory}
      />

      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <AlertTriangle className="size-4 text-amber-500" /> Clubes duplicados (nome normalizado
            igual) ({derived.duplicates.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {derived.duplicates.length === 0 ? (
            ...
            <div className="space-y-2">
              {derived.duplicates.map((group, i) => (
                <div
                  key={i}
                  className="flex flex-wrap gap-2 rounded-lg border border-border/60 p-2"
                >
                  {group.map((c) => (
                    <Link
                      key={c}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <AlertTriangle className="size-4 text-amber-500" /> Clubes com nomes semelhantes (
            {derived.similar.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {derived.similar.length === 0 ? (
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              ...
              {derived.similar.length > 100 && (
                <p className="text-xs text-muted-foreground">
                  … e mais {derived.similar.length - 100}
                </p>
              )}
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <AlertTriangle className="size-4 text-amber-500" /> (Época, Clube) sem país nos
            standings ({derived.noCountryRows.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {derived.noCountryRows.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Todos os (época, clube) têm país associado nos standings.
            </p>
          ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <AlertTriangle className="size-4 text-amber-500" /> Mais que um treinador na mesma época
            ({derived.multiCoachRows.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {derived.multiCoachRows.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Nenhum clube tem mais que um treinador na mesma época.
            </p>
          ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              Clubes em standings mas ausentes na tabela clubs ({derived.ghostClubs.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {derived.ghostClubs.map((c) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            Jogadores por clube (última época: {derived.latestYear || "—"})
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <RankCard
          title="Por pontuação ponderada"
          rows={ranked.slice(0, 100).map((e) => ({ name: e.name, value: Math.round(e.weighted) }))}
        />
        <RankCard
          title="Por pontuação bruta"
          rows={rankedRaw.slice(0, 100).map((e) => ({ name: e.name, value: Math.round(e.raw) }))}
        />
        <RankCard
          title="Por títulos"
          rows={rankedTitles.slice(0, 100).map((e) => ({ name: e.name, value: e.titles }))}
        />
      </div>
    </div>
  );
        ...
        {clubs.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1.5 max-h-[220px] overflow-y-auto">
            {clubs.slice(0, 400).map((c) => (
              <Link key={c} to="/clubes/$name" params={{ name: c }} search={{ tab: undefined }}>
            ...
            {clubs.length > 400 && (
              <span className="text-xs text-muted-foreground">… e mais {clubs.length - 400}</span>
            )}
          </div>
        )}
      </CardContent>
