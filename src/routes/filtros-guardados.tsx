      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold flex items-center gap-2">
            <FilterIcon className="size-6 text-primary" /> Filtros Guardados
          </h1>
          <p className="text-sm text-muted-foreground">
            Guarda combinações de filtros para reutilizar nos teus rankings.
          </p>
        </div>
              ...
              {list.map((f) => (
                <div key={f.id} className="flex items-start justify-between gap-3 py-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="sm" variant="outline" onClick={() => setDraft({ ...f })}>
                      Editar
                    </Button>
                    <Button
                      size="sm"
...
function Editor({
  ...
  draft: SavedFilter;
  setDraft: (f: SavedFilter | null) => void;
  onSave: () => void;
}) {
  ...
  const addFilter = () =>
    setDraft({
      ...draft,
      filters: [...draft.filters, { field: fields[0]?.key ?? "", op: ">=", value: "" }],
    });
  const updFilter = (i: number, patch: Partial<CustomFilter>) =>
    setDraft({
      ...draft,
      filters: draft.filters.map((f, idx) => (idx === i ? { ...f, ...patch } : f)),
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Configurar filtro</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <Select
              value={draft.entity}
              onValueChange={(v) => setDraft({ ...draft, entity: v as EntityKind, filters: [] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
