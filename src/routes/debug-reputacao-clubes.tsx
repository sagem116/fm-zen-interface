...
export const Route = createFileRoute("/debug-reputacao-clubes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Mapeamento de nomes longos para clubes canónicos e reputação por clube.",
      },
    ],
  }),
  ...
  const aliasRows = useMemo(
    () =>
      Object.entries(aliases)
        ...
        .sort((a, b) => a.raw.localeCompare(b.raw, "pt-PT")),
    [aliases],
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Reputação por clube, normalização de nomes longos e clubes ainda sem reputação.
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Adicionar reputação</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-wrap items-end gap-2">
          ...
          <Button
            ...
            onClick={() => {
              setReputation(newClub, Number(newRep));
              setNewClub("");
              setNewRep("");
            }}
          >
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Adicionar alias (nome longo → clube)</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-wrap items-end gap-2">
          <div>
          ...
          <Button
            ...
            onClick={() => {
              setClubAlias(newAliasRaw.trim(), newAliasCanon);
              setNewAliasRaw("");
              setNewAliasCanon("");
            }}
          >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button
          size="sm"
          variant={tab === "reps" ? "secondary" : "outline"}
          onClick={() => setTab("reps")}
        >
          Reputações ({repRows.length})
        </Button>
        <Button
          size="sm"
          variant={tab === "aliases" ? "secondary" : "outline"}
        ...
        <Button
          size="sm"
          variant={tab === "unmapped" ? "secondary" : "outline"}
          onClick={() => setTab("unmapped")}
        >
          Clubes sem reputação ({unmappedKnown.length})
        </Button>
              ...
              <tbody>
                {filtered(repRows, [(r) => r.club]).map((r) => (
                  <tr key={r.club} className="border-b border-border/50 hover:bg-muted/40">
                    ...
                    <td className="p-3">
                      {r.known ? (
                        <Badge variant="secondary">canónico</Badge>
                      ) : (
                        <Badge variant="outline">só reputação</Badge>
                      )}
                    </td>
                    ...
                    <td className="p-3 text-right">
                      <Button
                        ...
                        onClick={() => removeReputation(r.club)}
                      >
                ...
                {repRows.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-6 text-center text-muted-foreground">
                      Sem reputações. Importe um ficheiro em{" "}
                      <Link to="/importar" className="underline" search={{ tab: undefined }}>
                        Importar
                      </Link>
                      .
                    </td>
                  ...
                  <tr key={r.raw} className="border-b border-border/50 hover:bg-muted/40">
                    ...
                    <td className="p-3">
                      <Select
                        ...
                        onValueChange={(v) => {
                          if (v !== r.canonical) setClubAlias(r.raw, v);
                        }}
                      >
                ...
                {unmappedKnown.length === 0 && (
                  <tr>
                    <td colSpan={2} className="p-6 text-center text-muted-foreground">
                      Todos os clubes conhecidos têm reputação.
                    </td>
                  </tr>
                )}
              </tbody>
