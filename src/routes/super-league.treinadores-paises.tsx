  ...
  head: () => ({
    meta: [
      { title: "Treinadores por País — FM World Rankings" },
      {
        name: "description",
        content:
          "Pontos por nacionalidade do treinador em Super League, Continentais, Internacionais, Ligas Nacionais ou unificado.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  const { rows, epochs } = useMemo(() => {
    ...
    const addPts = (coachName: string | null | undefined, year: number, pts: number) => {
      ...
      let row = acc.get(nat);
      if (!row) {
        row = { pais: nat, perEpoch: {}, total: 0, coaches: new Map() };
        acc.set(nat, row);
      }
    ...
    if (wantCont) {
      if (contCompetition === "all") {
        ...
        for (const r of data.data.continental) {
          ...
          [r.qf1, r.qf2, r.qf3, r.qf4].forEach((t) =>
            stage(t, STAGE_RAW.quarter, cfg.stageMultipliers.quarter),
          );
        }
      }
    }
    ...
    if (wantIntl) {
      ...
      for (const r of data.data.international) {
        ...
        [r.sf1_coach, r.sf2_coach].forEach((c) =>
          stage(c, STAGE_RAW.semi, cfg.stageMultipliers.semi),
        );
        [r.qf1_coach, r.qf2_coach, r.qf3_coach, r.qf4_coach].forEach((c) =>
          stage(c, STAGE_RAW.quarter, cfg.stageMultipliers.quarter),
        );
      }
    }
  ...
  const historyChart = useMemo(() => {
    ...
    return allYears.map((y) => ({
      year: y,
      value: Math.round((r.perEpoch[y] ?? 0) * f * 100) / 100,
      name: r.pais,
    }));
  ...
  const clearFilters = () => {
    ...
    setSlDivision("all");
    setNlDivision("all");
    setContCompetition("all");
    setIntlCompetition("all");
  };
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-muted-foreground text-xs uppercase">
                    ...
                    {epochs.map((y) => (
                      <th key={y} className="text-right p-3">
                        <Sortable k={y} label={String(y)} />
                      </th>
                    ))}
                    <th className="text-right p-3">
                      <Sortable k="total" label="Total" />
                    </th>
                      ...
                      >
                        ...
                        {epochs.map((y) => {
                          ...
                          return (
                            <td
                              key={y}
                              className={`p-3 text-right tabular-nums ${v ? "" : "text-muted-foreground/30"}`}
                            >
                              {v ? fmtPts(v) : "—"}
                            </td>
                          );
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent side="left" className="max-w-xs">
                              <div className="font-semibold mb-1">
                                {r.pais} — Treinadores ({SCOPE_LABEL[scope]})
                              </div>
                              {topCoaches.length === 0 ? (
                                ...
                                <ul className="space-y-0.5">
                                  ...
                                  {topCoaches.length > 10 && (
                                    <li className="text-muted-foreground text-[10px] pt-1">
                                      +{topCoaches.length - 10} treinadores
                                    </li>
                                  )}
                                </ul>
                              )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex flex-wrap items-center gap-3 justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={focusName} onValueChange={setSelectedCountry}>
                  ...
                  <SelectContent>
                    {sorted.map((r) => (
                      <SelectItem key={r.pais} value={r.pais}>
                        {r.pais}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Evolução dos pontos por época para o âmbito e filtros selecionados (intervalo de
              épocas aplica-se).
            </p>
          </CardHeader>
          <CardContent>
