...
function InsightsPage() {
  ...
  const clearAll = () => {
    if (confirm("Eliminar todos os snapshots?")) {
      saveSnapshots([]);
      setSnaps([]);
    }
  };
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold flex items-center gap-2">
            <Sparkles className="size-6 text-primary" /> Insights
          </h1>
          <p className="text-sm text-muted-foreground">
            Resumo automático das mudanças entre a importação atual e a anterior. Cada importação
            adiciona um snapshot.
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {snaps.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Ainda não existem snapshots. Importa um ficheiro de jogadores para criar o primeiro.
            </p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {snaps.map((s, i) => (
                <Badge key={s.id} variant={i === snaps.length - 1 ? "default" : "outline"}>
                  #{i + 1} · {new Date(s.takenAt).toLocaleString("pt-PT")} · {s.totals.players} jog
                  · {s.source}
                </Badge>
              ))}
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Insights ({insights.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {insights.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              São necessários pelo menos 2 snapshots para gerar insights.
            </p>
          ) : (
