...
function FormulasPage() {
  ...
  const save = () => {
    ...
    const v = validateFormula(
      draft.expression,
      varsForEntity(draft.entity).map((x) => x.key),
    );
    if (!v.ok) {
      toast.error(`Fórmula inválida: ${v.error}`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold flex items-center gap-2">
            <FunctionSquare className="size-6 text-primary" /> Fórmulas Personalizadas
          </h1>
          <p className="text-sm text-muted-foreground">
            Cria métricas próprias usando as variáveis disponíveis para cada entidade.
          </p>
        </div>
        <Button onClick={startNew}>
          <Plus className="size-4" /> Nova fórmula
        </Button>
      </header>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              {list.find((x) => x.id === draft.id) ? "Editar" : "Criar"} fórmula
            </CardTitle>
          </CardHeader>
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Select
                  value={draft.entity}
                  onValueChange={(v) =>
                    setDraft({ ...draft, entity: v as EntityKind, expression: "" })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Fórmulas guardadas</CardTitle>
        </CardHeader>
        <CardContent>
          {list.length === 0 ? (
