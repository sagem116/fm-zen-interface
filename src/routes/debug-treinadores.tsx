...
export const Route = createFileRoute("/debug-treinadores")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Validar de onde vêm os títulos atribuídos a cada treinador.",
      },
    ],
  }),
  component: DebugPage,
...
function DebugPage() {
  ...
  const coachDiag = useMemo(() => {
    ...
    const byName = new Map<
      string,
      { seasons: Set<number>; clubs: Set<string>; nats: Set<string> }
    >();
    for (const c of coaches) {
      if (!c.name) continue;
      let e = byName.get(c.name);
      if (!e) {
        e = { seasons: new Set(), clubs: new Set(), nats: new Set() };
        byName.set(c.name, e);
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Diagnóstico de treinadores: dados em falta, duplicados, atribuições sem clube e trace de
            títulos por (época, módulo, clube).
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Stat
          label="Nacionalidade inconsistente"
          value={coachDiag?.duplicates.length ?? 0}
          tone="warn"
        />
      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {coachDiag.duplicates.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              {coachDiag.duplicates.length === 0 ? (
      ...
      <Tabs defaultValue="trace">
        <TabsList>
          ...
          <TabsTrigger value="orphan">
            Títulos sem treinador ({report.orphanTitleClubSeasons.length})
          </TabsTrigger>
        ...
        <TabsContent value="trace" className="space-y-3 mt-4">
          ...
          {rows.length > 200 && (
            <p className="text-xs text-muted-foreground">
              A mostrar os primeiros 200 de {rows.length}. Filtre por nome para ver mais.
            </p>
          )}
        </TabsContent>
        ...
        <TabsContent value="zero" className="space-y-3 mt-4">
          {withZero.length === 0 && (
            <p className="text-muted-foreground">Nenhum treinador com 0 títulos.</p>
          )}
        ...
        <TabsContent value="orphan" className="mt-4">
                    ...
                    <Tooltip key={i}>
                      <TooltipTrigger asChild>
                        <tr className="border-b border-border/40 hover:bg-muted/40 cursor-help">
                          ...
                          <td className="p-3">
                            <Badge variant="outline">{o.module}</Badge>
                          </td>
                      ...
                      <TooltipContent side="left">
                        <div className="text-xs">
                          <p className="font-medium">
                            {o.titles} título{o.titles === 1 ? "" : "s"} sem treinador
                          </p>
                          <p className="text-muted-foreground mt-1">
                            {o.season} · {o.module} · {o.club}
                          </p>
                        </div>
                      </TooltipContent>
                    </Tooltip>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Estes (época, módulo, clube) tiveram títulos mas <strong>nenhum treinador</strong> foi
            importado para essa atribuição, por isso ninguém herda esses títulos. Passe o rato sobre
            a linha para ver o resumo do título.
          </p>
        </TabsContent>
        ...
        <TabsContent value="skipped" className="mt-4">
          ...
          <div className="mt-2 p-3 rounded-md bg-muted/40 border border-border/60 text-xs space-y-1">
            <p>
              <strong>Origem do problema:</strong> na importação da época/módulo indicados o
              treinador ficou <em>sem clube atribuído</em>.
            </p>
            <p className="text-muted-foreground">
              Sem clube, o sistema não consegue cruzar a atribuição com o standings correspondente e
              por isso nenhum ponto/título lhe é herdado. Verifique os ficheiros de origem ou o{" "}
              <Link
                to="/debug-mapeamento-clubes"
...
function CoachChipList({ title, names }: { title: string; names: string[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <AlertTriangle className="size-4 text-amber-500" /> {title} ({names.length})
        </CardTitle>
      </CardHeader>
      <CardContent>
        {names.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1.5 max-h-[220px] overflow-y-auto">
            ...
            {names.length > 400 && (
              <span className="text-xs text-muted-foreground">… e mais {names.length - 400}</span>
            )}
          </div>
        )}
      </CardContent>
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <p
          className={`text-2xl font-bold tabular-nums mt-1 ${tone === "warn" && value > 0 ? "text-amber-500" : ""}`}
        >
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          {row.unmatchedAssignments.length > 0 && (
            <Badge variant="outline" className="text-amber-500 border-amber-500/40">
              <AlertTriangle className="size-3 mr-1" /> {row.unmatchedAssignments.length} sem
              standings
            </Badge>
          )}
        </div>
              ...
              <tbody>
                {row.sources.map((s, i) => (
                  <tr key={i} className="border-b border-border/30">
                    ...
                    <td
                      className={`p-2 text-right tabular-nums ${s.attributedTitles ? "font-semibold text-gold" : "text-muted-foreground"}`}
                    >
                      {s.attributedTitles}
                    </td>
                  </tr>
        ...
        {row.unmatchedAssignments.length > 0 && (
          <div className="mt-3 p-2 rounded-md bg-amber-500/5 border border-amber-500/20 text-xs">
            <p className="font-medium text-amber-500 mb-1">
              Atribuições sem standings correspondentes:
            </p>
            <ul className="text-muted-foreground space-y-0.5">
              {row.unmatchedAssignments.map((u, i) => (
                <li key={i}>
                  • {u.season} · {u.module} · {u.club}
                </li>
              ))}
            </ul>
          </div>
