  ...
  head: () => ({
    meta: [
      { title: "Jogadores por Divisão (Super League) — FM World Rankings" },
      {
        name: "description",
        content:
          "Médias de reputação, capacidade, idade, salários e valor por divisão da Super League.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  const sorted = useMemo(
    () =>
      [...rows].sort((a, b) => (sort === "division" ? a.division - b.division : b[sort] - a[sort])),
    [rows, sort],
  );
              ...
              <tr className="border-b border-border text-muted-foreground text-xs uppercase">
                <th className="text-left p-3">
                  <button
                    ...
                    className={`hover:text-foreground ${sort === "division" ? "text-foreground" : ""}`}
                  >
                    Divisão
                  </button>
                </th>
