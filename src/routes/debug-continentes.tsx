...
import { CONTINENTS, builtInContinentOf, continentOf, normalizeCountry } from "@/lib/fm-continents";
import {
...
export const Route = createFileRoute("/debug-continentes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Validação do mapeamento país → continente e edição de overrides.",
      },
    ],
  }),
...
function DebugContinentesPage() {
  ...
  const [filter, setFilter] = useState<"all" | "missing" | "overridden">("missing");
    ...
    return countries
      ...
      .filter((r) => {
        ...
        if (q.trim()) {
          ...
          return (
            r.country.toLowerCase().includes(s) || String(r.normalized).toLowerCase().includes(s)
          );
        }
        return true;
      });
