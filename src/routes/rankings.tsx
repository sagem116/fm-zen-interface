...
import { RankingsIndicatorsStrip } from "@/components/RankingsIndicatorsStrip";
import { RankingsInsightsPanel } from "@/components/RankingsInsightsPanel";
...
export const Route = createFileRoute("/rankings")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Rankings mundiais brutos e ponderados de clubes, treinadores e países.",
      },
    ],
  }),
      ...
      <CollapsibleContent>
        <Card className="bg-muted/40 border-dashed mt-2">
          <div className="p-4 text-sm space-y-2 text-muted-foreground">
            <p>
              <strong className="text-foreground">Pontos brutos</strong> — soma dos pontos base de
              cada posição, pontos conquistados nas ligas, bónus de campeão e pontos de títulos
              continentais.
            </p>
            <p>
              <strong className="text-foreground">Pontos ponderados</strong> — pontos brutos
              multiplicados por vários fatores:
            </p>
            <ul className="list-disc ml-5 space-y-1">
              <li>
                <strong>Peso da competição</strong> — SuperLeague, Ligas Nacionais e Continentais
                têm pesos diferentes.
              </li>
              <li>
                <strong>Peso da divisão/liga</strong> — divisões mais altas da SuperLeague e ligas
                nacionais configuradas valem mais.
              </li>
              <li>
                <strong>Desvalorização por época</strong> — épocas mais recentes têm multiplicador
                maior; resultados antigos perdem peso progressivamente.
              </li>
            </ul>
            <p>
...
function RankingsPage() {
  ...
  const [view, setView] = useState<"standard" | "players" | "competitions" | "clubs_stats">(
    "standard",
  );
  ...
  const [explainKind, setExplainKind] = useState<"clubes" | "treinadores" | "paises">("clubes");
  const [activeTab, setActiveTab] = useState<"clubs" | "coaches" | "countries">("clubs");
  const openExplain = (kind: "clubes" | "treinadores" | "paises") => (name: string) => {
    setExplainKind(kind);
    setExplainName(name);
    setExplainOpen(true);
  };
    ...
    const clubCols: ExtraCol[] = [
      {
        key: "promo",
        label: "Promovido",
        values: num(clubsCh, (r) => r.p),
        tips: tip(clubsCh, (r) => r.tipP),
      },
      {
        key: "despro",
        label: "Despromovido",
        values: num(clubsCh, (r) => r.d),
        tips: tip(clubsCh, (r) => r.tipD),
      },
      {
        key: "qsub",
        label: "Quase Subida",
        values: num(clubsPo, (r) => r.quaseSubida),
        tips: tip(clubsPo, (r) => r.tipQS),
      },
      {
        key: "qtit",
        label: "Quase Título",
        values: num(clubsPo, (r) => r.quaseTitulo),
        tips: tip(clubsPo, (r) => r.tipQT),
      },
    ];
    const coachCols: ExtraCol[] = [
      {
        key: "promo",
        label: "Promovido",
        values: num(coachesCh, (r) => r.p),
        tips: tip(coachesCh, (r) => r.tipP),
      },
      {
        key: "despro",
        label: "Despromovido",
        values: num(coachesCh, (r) => r.d),
        tips: tip(coachesCh, (r) => r.tipD),
      },
      {
        key: "qsub",
        label: "Quase Subida",
        values: num(coachesPo, (r) => r.quaseSubida),
        tips: tip(coachesPo, (r) => r.tipQS),
      },
      {
        key: "qtit",
        label: "Quase Título",
        values: num(coachesPo, (r) => r.quaseTitulo),
        tips: tip(coachesPo, (r) => r.tipQT),
      },
    ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Rankings históricos por competição e unificados
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {seasonView !== "total" && (
              <div className="flex rounded-lg border border-border p-1">
                <Button
                  size="sm"
                  variant={seasonScope === "cumulative" ? "default" : "ghost"}
                  onClick={() => setSeasonScope("cumulative")}
                >
                  Acumulado até
                </Button>
                <Button
                  size="sm"
                  variant={seasonScope === "only" ? "default" : "ghost"}
                  onClick={() => setSeasonScope("only")}
                >
                  Só essa época
                </Button>
              </div>
            ...
            <div
              ...
              title="Aplicar/Remover decaimento temporal nos rankings"
            >
              <Button
                size="sm"
                variant={decayMode === "with" ? "default" : "ghost"}
                onClick={() => setDecayMode("with")}
              >
                Com decaimento
              </Button>
              <Button
                size="sm"
                variant={decayMode === "without" ? "default" : "ghost"}
                onClick={() => setDecayMode("without")}
              >
                Sem decaimento
              </Button>
            </div>
            <div className="flex rounded-lg border border-border p-1" title="Densidade da tabela">
              <Button
                size="sm"
                variant={density === "comfy" ? "default" : "ghost"}
      ...
      {uiVersion === "v1" && (
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            size="sm"
            variant={view === "competitions" ? "default" : "outline"}
          ...
          <Button
            size="sm"
            variant={view === "clubs_stats" ? "default" : "outline"}
            onClick={() => setView("clubs_stats")}
          >
            Clubes (estatísticas)
          </Button>
        </div>
      ...
      {view === "players" && <PlayerRankingsView mode={mode} withDecay={decayMode === "with"} />}
      {view === "competitions" && (
        ...
        <>
              ...
              {ranks && (
                <>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => exportRankingsExcel(buildSections(ranks, mode))}
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      exportRankingsPDF(buildSections(ranks, mode), "FM World Rankings")
                    }
                  >
            ...
            <Card className="p-4">
              <div className="flex items-center gap-2 text-sm font-medium">
                <CollapsibleTrigger asChild>
                  <button
                    type="button"
                    className="flex items-center gap-2 hover:text-primary flex-1 text-left"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setContinentFilter(v === "all" ? "" : v)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  {moduleFilter === "international" && (
                    <>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={intlTeam}
                          onChange={(e) => setIntlTeam(e.target.value)}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={intlCoach}
                          onChange={(e) => setIntlCoach(e.target.value)}
                          placeholder="ex: Martínez"
                        />
                      </div>
                    </>
                  )}
            ...
            <>
              {intlRanks && (intlRanks.countries.length > 0 || intlRanks.coaches.length > 0) ? (
                <Tabs defaultValue="countries">
                  <TabsList>
                    <TabsTrigger value="countries">
                      <Globe2 className="size-3.5 mr-1" /> Seleções
                    </TabsTrigger>
                  ...
                  <TabsContent value="countries">
                    {(() => {
                      const v = sv(
                        filterEntries(intlRanks.countries, "paises"),
                        intlRanks.evolution.countries,
                        intlRanks.years,
                      );
                      return (
                        <SeasonsRankTable
                  ...
                  <TabsContent value="coaches">
                    {(() => {
                      const v = sv(
                        filterEntries(intlRanks.coaches, "treinadores"),
                        intlRanks.evolution.coaches,
                        intlRanks.years,
                      );
                      return (
                        <SeasonsRankTable
                ...
                <Card className="p-10 text-center text-sm text-muted-foreground">
                  {(data?.data.international?.length ?? 0) === 0 ? (
                    <>
                      Sem jogos de competições internacionais importados. Confirma que a folha{" "}
                      <em>Compts Seleções</em> tem linhas de jogos abaixo do cabeçalho.
                    </>
                  ) : (
            ...
            ranks && (
              <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
                ...
                <TabsContent value="clubs">
                  {(() => {
                    const v = sv(
                      filterEntries(ranks.clubs, "clubes"),
                      ranks.evolution.clubs,
                      ranks.years,
                    );
                    return (
                      <div className="space-y-3">
                        <RankingsIndicatorsStrip
                          entries={v.entries}
                          evolution={v.evolution}
                          years={v.years}
                          mode={mode}
                          onPick={openExplain("clubes")}
                        />
                        <RankingsInsightsPanel
                          entries={v.entries}
                          evolution={v.evolution}
                          years={v.years}
                ...
                <TabsContent value="coaches">
                  {(() => {
                    const v = sv(
                      filterEntries(ranks.coaches, "treinadores"),
                    ...
                    return (
                      <div className="space-y-3">
                        <RankingsIndicatorsStrip
                          ...
                          years={v.years}
                          mode={mode}
                          onPick={openExplain("treinadores")}
                        />
                        <RankingsInsightsPanel
                          entries={v.entries}
                          evolution={v.evolution}
                        ...
                        <SeasonsRankTable
                          ...
                          coachByKey={coachByKey}
                          onExplain={openExplain("treinadores")}
                        />
                      </div>
                    );
                  })()}
                ...
                <TabsContent value="countries">
                  {(() => {
                    const v = sv(
                      filterEntries(ranks.countries, "paises"),
          ...
          {data && ranks && (
            <RankingExplainSheet
              ...
              ranks={ranks}
              config={data.config}
              dataSummary={{ seasons: ranks.years.length }}
            />
          )}
        </>
      )}
...
function buildContextChips(a: ChipArgs): ContextChip[] {
  ...
  if (a.continentFilter)
    chips.push({
      key: "cont",
      label: `Continente: ${a.continentFilter}`,
      onClear: () => a.setContinentFilter(""),
    });
  if (a.countryFilter)
    chips.push({
      key: "country",
      label: `País: ${a.countryFilter}`,
      onClear: () => a.setCountryFilter(""),
    });
  if (a.nameSearch)
    chips.push({ key: "name", label: `Nome: ${a.nameSearch}`, onClear: () => a.setNameSearch("") });
  if (a.yearFrom !== "all")
    chips.push({ key: "yf", label: `Desde ${a.yearFrom}`, onClear: () => a.setYearFrom("all") });
  if (a.yearTo !== "all")
    chips.push({ key: "yt", label: `Até ${a.yearTo}`, onClear: () => a.setYearTo("all") });
  if (a.slDiv !== "all")
    chips.push({ key: "sl", label: `Divisão ${a.slDiv}`, onClear: () => a.setSlDiv("all") });
  if (a.natComp !== "all")
    chips.push({ key: "nc", label: a.natComp, onClear: () => a.setNatComp("all") });
  if (a.contComp !== "all")
    chips.push({ key: "cc", label: a.contComp, onClear: () => a.setContComp("all") });
  if (a.intlComp !== "all")
    chips.push({ key: "ic", label: a.intlComp, onClear: () => a.setIntlComp("all") });
  if (a.intlTeam)
    chips.push({ key: "it", label: `Seleção: ${a.intlTeam}`, onClear: () => a.setIntlTeam("") });
  if (a.intlCoach)
    chips.push({
      key: "ico",
      label: `Treinador: ${a.intlCoach}`,
      onClear: () => a.setIntlCoach(""),
    });
  return chips;
}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-xs mt-1">
            UI Moderna · Filtros, contexto e âmbito num só lugar
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Popover>
            ...
            <PopoverContent align="end" className="w-[300px] space-y-3">
              <div>
                ...
                <div className="flex rounded-lg border border-border p-1 mt-1">
                  <Button
                    size="sm"
                    variant={props.mode === "weighted" ? "default" : "ghost"}
                    className="flex-1"
                    onClick={() => props.setMode("weighted")}
                  >
                    Ponderado
                  </Button>
                  <Button
                    size="sm"
                    variant={props.mode === "raw" ? "default" : "ghost"}
                    className="flex-1"
                    onClick={() => props.setMode("raw")}
                  >
                    Bruto
                  </Button>
                </div>
              ...
              <div>
                ...
                <div className="flex rounded-lg border border-border p-1 mt-1">
                  <Button
                    size="sm"
                    variant={props.decayMode === "with" ? "default" : "ghost"}
                    className="flex-1"
                    onClick={() => props.setDecayMode("with")}
                  >
                    Com
                  </Button>
                  <Button
                    size="sm"
                    variant={props.decayMode === "without" ? "default" : "ghost"}
                    className="flex-1"
                    onClick={() => props.setDecayMode("without")}
                  >
                    Sem
                  </Button>
                </div>
              ...
              <div>
                ...
                {props.seasonView !== "total" && (
                  <div className="flex rounded-lg border border-border p-1 mt-2">
                    <Button
                      size="sm"
                      variant={props.seasonScope === "cumulative" ? "default" : "ghost"}
                      className="flex-1"
                      onClick={() => props.setSeasonScope("cumulative")}
                    >
                      Acumulado
                    </Button>
                    <Button
                      size="sm"
                      variant={props.seasonScope === "only" ? "default" : "ghost"}
                      className="flex-1"
                      onClick={() => props.setSeasonScope("only")}
                    >
                      Só essa
                    </Button>
                  </div>
