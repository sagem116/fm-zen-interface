  ...
  head: () => ({
    meta: [
      { title: "Jogadores por Clube (Super League) — FM World Rankings" },
      {
        name: "description",
        content:
          "Médias de reputação, capacidade, idade, salários e valor de plantel por clube da Super League.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  const divOptions = useMemo(
    () =>
      Array.from(new Set(rows.map((r) => r.division).filter((d) => d != null))).sort(
  ...
  const hasFilters =
    search || leagueFilter !== "all" || divFilter !== "all" || countryFilter !== "all";
