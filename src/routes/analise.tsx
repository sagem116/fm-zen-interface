...
export const Route = createFileRoute("/analise")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Descobre tendências, records e curiosidades do universo Football Manager.",
      },
      { property: "og:title", content: "Análise Estatística — FM World Rankings" },
      {
        property: "og:description",
        content: "Rankings avançados, evolução histórica, records e curiosidades.",
      },
    ],
  }),
    ...
    <Card className="card-glow">
      ...
      <CardContent className="space-y-0.5">
        {r.rows.map((row, i) => (
          <div
            key={`${row.name}-${i}`}
            className="flex items-center gap-2 py-1 text-sm border-b border-border/40 last:border-0"
          >
            <span
          ...
          <CardContent className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={s.points} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(v: number) => fmt(v, s.fmt)}
  ...
  if (!rows.length)
    return (
      <p className="text-sm text-muted-foreground">
        Sem registos suficientes para calcular records.
      </p>
    );
  return (
...
function CuriositiesTab({ items }: { items: ReturnType<typeof computeCuriosities> }) {
  if (!items.length)
    return (
      <p className="text-sm text-muted-foreground">
        É necessária mais do que uma época para gerar curiosidades.
      </p>
    );
  return (
    <div className="grid gap-3 md:grid-cols-2">
...
function AnalisePage() {
  ...
  const compsData = useMemo(
    () => (data ? competitionRankings(data, seasonSet) : []),
    [data, seasonSet],
  );
  const playersData = useMemo(
    () => (data ? playerRankings(data, seasonSet) : []),
    [data, seasonSet],
  );
  const coachesData = useMemo(
    () => (data ? coachRankings(data, seasonSet) : []),
    [data, seasonSet],
  );
  const countriesData = useMemo(
    () => (data ? countryRankings(data, seasonSet) : []),
    [data, seasonSet],
  );
      ...
      <Tabs defaultValue="clubes" className="space-y-4">
        <TabsList className="flex flex-wrap h-auto">
          ...
          <TabsTrigger value="evolucao">
            <LineIcon className="size-3.5 mr-1" />
            Evolução
          </TabsTrigger>
          ...
          <TabsTrigger value="curiosidades">
        ...
        <TabsContent value="clubes" className="space-y-6">
          {clubsData.map((g) => (
            <RankingGroup key={g.group} title={g.group} rankings={g.rankings} />
          ))}
        </TabsContent>
        ...
        <TabsContent value="evolucao">
          <EvolutionTab series={evoData} />
        </TabsContent>
        <TabsContent value="records">
          <RecordsTab rows={recordsData} />
        </TabsContent>
        <TabsContent value="curiosidades">
          <CuriositiesTab items={curiosData} />
        </TabsContent>
      </Tabs>
    </div>
  );
