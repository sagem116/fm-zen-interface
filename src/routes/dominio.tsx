  ...
  for (let i = 0; i < years.length; i += windowSize) {
    ...
    for (const y of slice) {
      for (const r of byYear.get(y) ?? []) {
        const cur = agg.get(r.club_name) ?? {
          name: r.club_name,
          pts: 0,
          titulos: 0,
          promos: 0,
          linkTo: "club" as const,
        };
  ...
  for (let i = 0; i < years.length; i += windowSize) {
    ...
    for (const y of slice) {
      for (const r of byYear.get(y) ?? []) {
        ...
        const cur = agg.get(coach) ?? {
          name: coach,
          pts: 0,
          titulos: 0,
          promos: 0,
          linkTo: "coach" as const,
        };
        if (scoring === "raw") cur.pts += Number(r.points ?? 0) || 0;
        const tk = tokens(r.info);
        if (r.is_champion || tk.has("C")) cur.titulos++;
...
function DominioPage() {
  ...
  const windows = useMemo(() => {
    if (category === "clubs") {
      return buildClubWindows(standings, breakdownClubs, module, scoring, windowSize, allowedClubs);
    }
        ...
        <Card>
          ...
          <CollapsibleContent>
            <CardContent className="text-sm space-y-3 text-muted-foreground">
              <p>
                As épocas são agrupadas em janelas consecutivas do tamanho escolhido (ex: 10
                épocas). Dentro de cada janela, para cada clube ou treinador somam-se:
              </p>
              <ul className="list-disc pl-5 space-y-1.5">
                <li>
                  <strong className="text-foreground">Títulos</strong> — épocas em que foi campeão
                  (campo <code>is_champion</code> ou marcador <code>C</code> no info da época). Para
                  treinadores conta-se o título da época em que treinou o clube campeão.
                </li>
                <li>
                ...
                <li>
                  <strong className="text-foreground">Pontos brutos</strong> — soma simples dos
                  pontos de liga em cada época da janela.
                </li>
                <li>
                  <strong className="text-foreground">Pontos ponderados</strong> — soma dos pontos
                  calculados pelos pesos ativos (divisão, competição, decaimento por época, bónus de
                  campeão, etc.), coerentes com os Rankings Mundiais.
                </li>
              </ul>
              <p>
                Ordenação: <strong>títulos</strong> (desempate por <strong>pontos</strong>) — top 10
                por janela.
              </p>
              <p className="text-xs">
                Filtros: clube, país e treinador restringem o conjunto considerado (correspondência
                parcial, sem acentos). Na vista de treinadores, os filtros de clube/país restringem
                os treinadores àqueles que treinaram clubes correspondentes.
              </p>
              <p className="text-xs">
