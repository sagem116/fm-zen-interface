...
type Key = keyof Pick<NationalLeagueAgg, "ra" | "rm" | "ca" | "cp" | "age" | "salary" | "vp" | "n">;
...
function Page() {
  ...
  const rows = useMemo(
    () =>
      data ? computeNationalLeagueAggregates(data.data.players, data.data.standings, year) : [],
    [data, year],
  );
  ...
  const playerMatches = useMemo(() => {
    ...
    for (const p of universe.list ?? []) {
      if (!p?.name || !p.currentClub || !nationalClubs.has(p.currentClub)) continue;
      if (!normalize(p.name).includes(q)) continue;
      out.push({
        key: `${p.idu ?? p.uid}|${p.currentClub}|${p.seasonYear ?? 0}`,
        name: p.name,
        club: p.currentClub,
        league: ((p.extras as any)?.league as string | null) ?? null,
        year: p.seasonYear ?? 0,
        ca: p.ca ?? 0,
      });
    }
    ...
    for (const item of out) {
      const key = `${normalize(item.name)}|${normalize(item.club ?? "")}|${item.year}`;
      if (!dedup.has(key)) dedup.set(key, item);
    }
    const finalRows = [...dedup.values()];
    // Most recent + best first
    finalRows.sort((a, b) => b.year - a.year || b.ca - a.ca || a.name.localeCompare(b.name));
    return finalRows;
  }, [data, deferredSearch, nationalClubs, universe.list]);
  ...
  if (!rows.length)
    return (
      <p className="text-muted-foreground">
        Sem dados de jogadores em ligas nacionais. Importa um ficheiro de Liga Nacional com a folha
        "Jogadores".
      </p>
    );
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground max-w-3xl rounded-lg border border-border bg-muted/40 p-3 leading-relaxed">
          Indicadores de jogadores agregados por liga nacional: médias de R.A., R.M., C.A. e C.P.
          (28 melhores por clube), idade média e somas de salários e valor. Filtra por época,
          pesquisa um jogador, ou navega pelas ligas com paginação.
        </p>
      </div>
