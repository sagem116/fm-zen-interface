  ...
  head: () => ({
    meta: [
      { title: "Sugestão de Pesos — FM World Rankings" },
      {
        name: "description",
        content:
          "Sugestão automática de pesos para competições com base numa fórmula configurável.",
      },
    ],
  }),
...
function SugestaoPesosPage() {
  ...
  const [sortKey, setSortKey] = useState<"competition" | "index" | "weight" | "current" | "diff">(
    "weight",
  );
  ...
  const minMax = useMemo(() => {
    const mm: Record<MetricKey, { min: number; max: number }> = {} as Record<
      MetricKey,
  ...
  const rows = useMemo(() => {
    const sumW = METRICS.reduce(
      (s, m) => s + (formula.enabled[m.key] ? Math.abs(formula.weights[m.key]) : 0),
      0,
    );
    ...
    if (formula.normalize && sumW > 0) {
      ...
      for (const r of result) {
        if (r.index < imin) imin = r.index;
        if (r.index > imax) imax = r.index;
      }
      const rng = imax - imin;
      suggested = result.map((r) => ({
        row: r,
        suggested: rng > 0 ? ((r.index - imin) / rng) * formula.scale : 0,
      }));
    }
  ...
  const setMetric = (k: MetricKey, patch: Partial<{ enabled: boolean; weight: number }>) => {
    setFormula((f) => {
      let nextEnabled =
        patch.enabled !== undefined ? { ...f.enabled, [k]: patch.enabled } : f.enabled;
      let nextWeights =
        patch.weight !== undefined ? { ...f.weights, [k]: patch.weight } : f.weights;
  ...
  const handleSort = (k: typeof sortKey) => {
    if (sortKey === k) setSortDir((d) => (d === "asc" ? "desc" : "asc"));
      ...
      setSortKey(k);
      setSortDir(k === "competition" ? "asc" : "desc");
    }
  };
  ...
  const resetFormula = () => {
    ...
    toast.success("Fórmula restaurada");
  };
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `formula-${formula.name}.json`;
    a.click();
  ...
  const compareData = top15.map((r) => ({
    name: r.competition,
    atual: Number(r.current.toFixed(2)),
    sugerido: Number(r.suggested.toFixed(2)),
  }));
  const distribution = useMemo(() => {
    ...
    const buckets = Array.from({ length: bins }, (_, i) => ({
      bin: `${(min + i * w).toFixed(1)}–${(min + (i + 1) * w).toFixed(1)}`,
      n: 0,
    }));
    for (const v of vals) {
  ...
  if (!aggregates.length) {
    return (
      <p className="text-muted-foreground">
        Sem dados de competições. Importa primeiro um XLSX multi-folha em{" "}
        <strong>Importar Época</strong>.
      </p>
    );
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Calcula automaticamente um peso para cada competição com base na qualidade média dos
            jogadores.
          </p>
        </div>
      </div>
      ...
      <Card className="p-4 space-y-4">
        <div className="flex flex-wrap items-end gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 h-9">
              <Switch
                checked={formula.normalize}
                onCheckedChange={(v) => setFormula((f) => ({ ...f, normalize: v }))}
              />
          ...
          <div>
            ...
            <Select
              value={String(formula.scale)}
              onValueChange={(v) => setFormula((f) => ({ ...f, scale: Number(v) as Scale }))}
            >
              ...
              <SelectContent>
                {SCALES.map((s) => (
                  <SelectItem key={s} value={String(s)}>{`0–${s}`}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div>
            ...
            <Select
              value={String(formula.maxActive ?? 0)}
              onValueChange={(v) => setMaxActive(Number(v))}
            >
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" onClick={saveFormula}>
              <Save className="size-4 mr-1" /> Guardar
            </Button>
            <Button size="sm" variant="outline" onClick={duplicateFormula}>
              <Copy className="size-4 mr-1" /> Duplicar
            </Button>
            <Button size="sm" variant="outline" onClick={resetFormula}>
              <RotateCcw className="size-4 mr-1" /> Padrão
            </Button>
            ...
            <label>
              ...
              <Button size="sm" variant="outline" asChild>
                <span>
                  <UploadIcon className="size-4 mr-1" /> Importar
                </span>
              </Button>
            </label>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {METRICS.map((m) => (
            <div
              key={m.key}
              className={`rounded-md border p-3 ${formula.enabled[m.key] ? "border-gold/40 bg-gold/5" : "border-border"}`}
            >
              <div className="flex items-center justify-between">
                <Label className="font-semibold">{m.label}</Label>
                <Switch
                  checked={formula.enabled[m.key]}
                  onCheckedChange={(v) => setMetric(m.key, { enabled: v })}
                />
              </div>
              <Input
                type="number"
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setCompTypeFilter(v as CompType | "all")}
            >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Valores guardados na base de dados e disponíveis como métrica nas fórmulas.
            </p>
          </div>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {aggregates
                .filter(
                  (a) =>
                    !search.trim() || a.competition.toLowerCase().includes(search.toLowerCase()),
                )
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead onClick={() => handleSort("diff")} className="cursor-pointer text-right">
                  Diferença
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {filteredRows.map((r) => (
                <TableRow key={`${r.comp_type}|${r.competition}`}>
                  ...
                  <TableCell className="text-right font-bold text-gold">
                    {fmtNum(r.suggested, 2)}
                  </TableCell>
                  <TableCell className="text-right">{fmtNum(r.current, 2)}</TableCell>
                  <TableCell
                    className={`text-right ${r.diff > 0 ? "text-emerald-500" : r.diff < 0 ? "text-rose-500" : ""}`}
                  >
                    {r.diff > 0 ? "+" : ""}
                    {fmtNum(r.diff, 2)}
                  </TableCell>
          ...
          <div style={{ width: "100%", height: 320 }}>
            <ResponsiveContainer>
              <BarChart
                data={top15.map((r) => ({
                  name: r.competition,
                  peso: Number(r.suggested.toFixed(2)),
                }))}
                margin={{ left: 4, right: 12, top: 8, bottom: 60 }}
              >
                ...
                <Bar dataKey="peso" fill="hsl(var(--primary))">
                  {top15.map((_, i) => (
                    <Cell key={i} fill={`hsl(${42 - i * 2} 80% ${55 - i}%)`} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
