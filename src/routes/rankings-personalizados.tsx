...
import {
  ...
  upsertCustomRanking,
  deleteCustomRanking,
  duplicateCustomRanking,
  newCustomRankingId,
  applyFilter,
  type CustomRanking,
  type CustomFilter,
  type FilterOp,
} from "@/lib/fm-custom-rankings";
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-display font-bold flex items-center gap-2">
            <Trophy className="size-6 text-primary" /> Rankings Personalizados
          </h1>
          <p className="text-sm text-muted-foreground">
            Cria os teus próprios rankings com filtros e ordenação personalizada.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link to="/formulas-personalizadas" search={{ tab: undefined }}>
            <Button variant="outline">Fórmulas</Button>
          </Link>
          <Link to="/filtros-guardados" search={{ tab: undefined }}>
            <Button variant="outline">Filtros Guardados</Button>
          </Link>
          <Button onClick={() => setDraft(emptyRanking())}>
      ...
      {draft && (
        <RankingEditor
          ...
          onSave={(r) => {
            upsertCustomRanking(r);
            toast.success(`"${r.name}" guardado`);
            setDraft(null);
          }}
        />
      )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Rankings guardados</CardTitle>
        </CardHeader>
        <CardContent>
          {list.length === 0 ? (
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="secondary">
                        Ord: {r.orderBy} {r.orderDir === "desc" ? "↓" : "↑"}
                      </Badge>
                      {r.filters.length > 0 && (
                        <Badge variant="secondary">{r.filters.length} filtro(s)</Badge>
                      )}
                    </div>
                    {r.description && (
...
function RankingEditor({
  ...
  const addFilter = () =>
    setR({
      ...r,
      filters: [...r.filters, { field: filterFields[0]?.key ?? "", op: ">=", value: "" }],
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Configurar ranking</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid sm:grid-cols-3 gap-3">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setR({
                  ...
                  filters: [],
                  orderBy: varsForEntity(v as EntityKind)[0]?.key ?? "",
                })
              }
            >
              <SelectTrigger>
          ...
          <div>
            ...
            <Input
              ...
              min={0}
              value={r.limit ?? 0}
              onChange={(e) => setR({ ...r, limit: Math.max(0, Number(e.target.value) || 0) })}
            />
          </div>
        </div>
        <div>
        ...
        <div className="space-y-2">
          ...
          {r.filters.length === 0 && (
            <p className="text-xs text-muted-foreground">
              Nenhum filtro — todas as entidades incluídas.
            </p>
          )}
          {r.filters.map((f, i) => (
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Select
              value={r.orderDir}
              onValueChange={(v) => setR({ ...r, orderDir: v as "asc" | "desc" })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        <div className="flex items-center gap-2">
          <Button onClick={() => onSave({ ...r, updatedAt: Date.now() })}>
...
function RankingResults({ ranking, onClose }: { ranking: CustomRanking; onClose: () => void }) {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = all.filter((row) => {
      for (const f of ranking.filters) {
        const merged = { ...row.ctx, ...row.meta } as Record<
          string,
  ...
  const orderLabel = ranking.orderBy.startsWith("FORMULA:")
