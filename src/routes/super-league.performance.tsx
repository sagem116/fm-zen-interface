...
export const Route = createFileRoute("/super-league/performance")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Resumo de golos, assistências, salário e valor por jogador na Super League.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  const withUniverse = useMemo(
    () =>
      sorted.map((r) => {
        const uni = universe.getByName(r.name);
        if (!uni) return r;
        const years = Object.keys(uni.history ?? {})
          .map(Number)
          .filter((y) => Number.isFinite(y))
          .sort((a, b) => b - a);
        const latest = years.length ? uni.history?.[years[0]] : null;
        return {
          ...r,
          club: uni.currentClub ?? r.club,
          age: uni.age ?? r.age,
          salary: Number(latest?.salary ?? r.salary ?? 0),
          vp: Number(latest?.value ?? r.vp ?? 0),
        } as PerformanceRow;
      }),
    [sorted, universe],
  );
  ...
  if (!rows.length)
    return (
      <p className="text-muted-foreground">
        Sem dados de jogadores. Importa um ficheiro da Super League com a folha "Jogadores".
      </p>
    );
      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {withUniverse.slice(0, 300).map((r, i) => (
                <tr key={r.name + i} className="border-b border-border/50 hover:bg-muted/50">
                  <td className={`p-3 font-bold ${i < 3 ? "text-gold" : "text-muted-foreground"}`}>
                    {i + 1}
                  </td>
