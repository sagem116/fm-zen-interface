  ...
  head: () => ({
    meta: [
      { title: "Histórico de Rankings — FM World Rankings" },
      {
        name: "description",
        content:
          "Posição que cada clube, treinador e país ocupou no ranking mundial em cada época.",
      },
    ],
  }),
...
function HistoricoRankings() {
  ...
  const overallClubs =
    source === "all" ? rankBy(data.ranks.clubs, mode).map((e) => e.name) : deriveOverall(clubYM);
  const overallCoaches =
    source === "all" ? rankBy(data.ranks.coaches, mode).map((e) => e.name) : deriveOverall(coachYM);
  const overallCountries =
    source === "all"
      ? rankBy(data.ranks.countries, mode).map((e) => e.name)
      : deriveOverall(countryYM);
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <HistoryIcon className="size-6 text-primary" /> Histórico de Rankings
        </h1>
        <p className="text-sm text-muted-foreground">
          Posição que cada entidade ocupou no Ranking ({sourceLabel}) em cada época, com pontos{" "}
          {mode === "weighted" ? "ponderados" : "brutos"}.
        </p>
      </header>
        ...
        <CardContent className="pt-6 flex flex-wrap items-end gap-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <select
              ...
              onChange={(e) =>
                setYearFrom(e.target.value === "min" ? "min" : Number(e.target.value))
              }
        ...
        <TabsList>
          {source !== "international" && (
            <TabsTrigger value="clubes">
              <Shield className="size-4 mr-1" /> Clubes
            </TabsTrigger>
          )}
          <TabsTrigger value="treinadores">
            <Users className="size-4 mr-1" /> Treinadores
          </TabsTrigger>
          <TabsTrigger value="paises">
            <Globe2 className="size-4 mr-1" /> {source === "international" ? "Seleções" : "Países"}
          </TabsTrigger>
        </TabsList>
        {source !== "international" && (
                ...
                <tr key={name} className="border-b hover:bg-muted/40">
                  ...
                  {years.map((y) => {
                    ...
                    return (
                      <td
                        key={y}
                        className={`px-2 py-1.5 text-center tabular-nums ${rankClass(r)}`}
                      >
                        {r != null ? r : "·"}
                      </td>
                    );
