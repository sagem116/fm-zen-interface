...
import {
  setManualClubMapping,
  removeManualClubMapping,
  getManualClubMappings,
  type ClubMapping,
} from "@/lib/fm-club-map";
...
function DebugMapeamentoClubes() {
  ...
  const seasons = useMemo(() => {
    ...
    details.forEach((d) => {
      if (d.season != null) s.add(d.season);
    });
  ...
  const counts = useMemo(() => {
    const c: Record<Flag, number> = {
      sem_competicao: 0,
      duplicado: 0,
      multiplas_competicoes: 0,
      pais_inconsistente: 0,
      nao_encontrado: 0,
    };
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `mapeamento-clubes-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Fonte oficial: Super Leagues + Ligas Nacionais. Um jogador emprestado nunca reclassifica
            o clube destino.
          </p>
        </div>
        ...
        {(Object.keys(FLAG_LABEL) as Flag[]).map((f) => (
          <Card key={f}>
            <CardContent className="pt-6">
              <p
                className={`text-2xl font-bold tabular-nums ${counts[f] ? "text-amber-500" : "text-success"}`}
              >
                {counts[f]}
              </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <AlertTriangle className="size-4 text-amber-500" /> Resolução manual de conflitos (
              {conflicts.length})
            </CardTitle>
          </CardHeader>
                ...
                <div key={key} className="rounded-lg border border-border p-3">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    {chosen && (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => clearOverride(c.season, c.club)}
                      >
                  ...
                  <div className="grid gap-2 md:grid-cols-2">
                    {c.candidates.map((cand, i) => {
                      const isChosen =
                        chosen &&
                        chosen.competition === cand.competition &&
                        chosen.comp_type === cand.comp_type;
                      return (
                        <button
                          key={i}
