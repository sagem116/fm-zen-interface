...
export const Route = createFileRoute("/configuracao")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Pesos de competições, divisões, títulos e fórmula mundial editáveis.",
      },
    ],
  }),
    ...
    <div className="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      {state.key !== "original" && (
        <Button
          ...
          variant="ghost"
          className="h-7 px-2 text-xs"
          onClick={() => onChange({ key: "original", dir: "asc" })}
        >
          Repor
        </Button>
      )}
...
function sortIndices<T extends { label: string; weight: number }>(
  arr: T[],
  s: SortState,
): number[] {
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            ...
            usando a configuração atual <em>sem</em> guardar nem recalcular os rankings. Fórmula:{" "}
            <code>raw = peso × base_fase</code> ·{" "}
            <code>ponderado = raw × pesoComp × multFase × decaimento</code>.
          </p>
        </div>
      ...
      {open && (
        <CardContent className="space-y-3">
                  ...
                  {grouped.map(([key, group]) =>
                    group.map((r, i) => (
                      ...
                      >
                        {i === 0 ? (
                          <>
                            <td className="px-2 py-1.5" rowSpan={group.length}>
                              {r.scope}
                            </td>
                            <td className="px-2 py-1.5 font-medium" rowSpan={group.length}>
                              {r.competition}
                            </td>
                            <td className="px-2 py-1.5 text-right" rowSpan={group.length}>
                              {fmt(r.weight)}
                            </td>
                          </>
                        ) : null}
          ...
          <p className="text-[11px] text-muted-foreground">
            Decay simulado = última época ({fmt(cfg.decayMultipliers.last)}). Para épocas mais
            antigas, multiplica pelo decay correspondente.
          </p>
        </CardContent>
      )}
    </Card>
...
function ConfigPage() {
  ...
  const refresh = () => {
    ...
    qc.invalidateQueries({ queryKey: ["fm-rankings"] });
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (id === activeId && profiles[0])
        await activateProfile(profiles.find((p) => p.id !== id)!.id);
  ...
  const handleImport = (file: File) => {
    ...
    reader.onload = () => {
      try {
            ...
            ? arr.map(
                (t: {
                  ...
                  if (
                    withPosBonuses &&
                    t.positionBonuses &&
                    typeof t.positionBonuses === "object"
                  ) {
        ...
        if (parsed.competitionWeights)
          merged.competitionWeights = {
            ...merged.competitionWeights,
            ...parsed.competitionWeights,
          };
        ...
        if (typeof parsed.nationalChampionBonus === "number")
          merged.nationalChampionBonus = parsed.nationalChampionBonus;
        if (typeof parsed.superleagueChampionBonus === "number")
          merged.superleagueChampionBonus = parsed.superleagueChampionBonus;
        if (typeof parsed.superleaguePromotionBonus === "number")
          merged.superleaguePromotionBonus = parsed.superleaguePromotionBonus;
        if (typeof parsed.dobradinhaBonus === "number")
          merged.dobradinhaBonus = parsed.dobradinhaBonus;
        if (typeof parsed.dobradinhaInternacionalBonus === "number")
          merged.dobradinhaInternacionalBonus = parsed.dobradinhaInternacionalBonus;
        if (typeof parsed.tripleteBonus === "number") merged.tripleteBonus = parsed.tripleteBonus;
        if (typeof parsed.quadrupleBonus === "number")
          merged.quadrupleBonus = parsed.quadrupleBonus;
        ...
        if (parsed.decayMultipliers)
          merged.decayMultipliers = { ...merged.decayMultipliers, ...parsed.decayMultipliers };
        if (parsed.stageMultipliers)
          merged.stageMultipliers = { ...merged.stageMultipliers, ...parsed.stageMultipliers };
        if (typeof parsed.normalizePointsByGames === "boolean")
          merged.normalizePointsByGames = parsed.normalizePointsByGames;
        setCfg(merged);
  ...
  const handleAutoPopulate = async () => {
    try {
      ...
      const contNames = [
        ...new Set((cont ?? []).map((r) => r.competition).filter(Boolean) as string[]),
      ];
      const intlNames = [
        ...new Set((intl ?? []).map((r) => r.competition).filter(Boolean) as string[]),
      ];
      const natNames = [
        ...new Set((nat ?? []).map((r) => r.division_label).filter(Boolean) as string[]),
      ];
  ...
  return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Pesos, bónus, desvalorização e fórmula mundial
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" onClick={handleAutoPopulate}>
            <Sparkles className="size-4" /> Popular competições
          </Button>
          <Button
            variant="outline"
            onClick={() => {
    ...
    qc.invalidateQueries({ queryKey: ["fm-rankings"] });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Perfis de configuração</CardTitle>
        </CardHeader>
      ...
      <div className="grid gap-4">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Pesos por competição (Fórmula Mundial)</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <NumField
              label="SuperLeague"
              step={0.1}
              value={cfg.competitionWeights.superleague}
              onChange={(v) =>
                upd((c) => {
                  c.competitionWeights.superleague = v;
                })
              }
            />
            <NumField
              label="Continental"
              step={0.1}
              value={cfg.competitionWeights.continental}
              onChange={(v) =>
                upd((c) => {
                  c.competitionWeights.continental = v;
                })
              }
            />
            <NumField
              label="Nacional"
              step={0.1}
              value={cfg.competitionWeights.national}
              onChange={(v) =>
                upd((c) => {
                  c.competitionWeights.national = v;
                })
              }
            />
            <NumField
              label="Seleções"
              step={0.1}
              value={cfg.competitionWeights.international ?? 1.5}
              onChange={(v) =>
                upd((c) => {
                  c.competitionWeights.international = v;
                })
              }
            />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              Bónus de campeão, promoção e múltiplas conquistas
            </CardTitle>
            <p className="text-xs text-muted-foreground mt-1">
              Bónus de promoção: atribuído quando uma época SuperLeague tem indicador "P" (subiu de
              divisão). Dobradinha = SL + Liga Nacional · Dobradinha Internacional = SL +
              Continental · Triplete = SL + Liga Nacional + Continental · Quadruple = Liga Nacional
              + SL + Continental + Club World Cup. Apenas o bónus de maior prioridade (Quadruple{" "}
              {">"} Triplete {">"} Dobradinha Int. {">"} Dobradinha) é atribuído por clube/época.
            </p>
          </CardHeader>
          <CardContent className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <NumField
              label="Campeão SuperLeague"
              value={cfg.superleagueChampionBonus}
              onChange={(v) =>
                upd((c) => {
                  c.superleagueChampionBonus = v;
                })
              }
            />
            <NumField
              label="Campeão Nacional"
              value={cfg.nationalChampionBonus}
              onChange={(v) =>
                upd((c) => {
                  c.nationalChampionBonus = v;
                })
              }
            />
            <NumField
              label="Promoção SuperLeague"
              value={cfg.superleaguePromotionBonus}
              onChange={(v) =>
                upd((c) => {
                  c.superleaguePromotionBonus = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Dobradinha (SL + Nacional)"
              value={cfg.dobradinhaBonus}
              onChange={(v) =>
                upd((c) => {
                  c.dobradinhaBonus = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Dobradinha Internacional (SL + Continental)"
              value={cfg.dobradinhaInternacionalBonus}
              onChange={(v) =>
                upd((c) => {
                  c.dobradinhaInternacionalBonus = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Triplete (SL + Nacional + Continental)"
              value={cfg.tripleteBonus}
              onChange={(v) =>
                upd((c) => {
                  c.tripleteBonus = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Quadruple (NL + SL + Cont + CWC)"
              value={cfg.quadrupleBonus}
              onChange={(v) =>
                upd((c) => {
                  c.quadrupleBonus = Math.max(0, v);
                })
              }
            />
          </CardContent>
        ...
        {(["triplete", "dob-int", "quad-cont", "quad-cwc"] as const).map((kind) => {
          const meta = {
            ...
            "quad-cwc": {
              ...
              fallbackText:
                "Se vazio, é feita correspondência automática por 'club world cup' no nome.",
            },
          }[kind];
          ...
          return (
            <Card key={kind}>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Correspondência por substring (sem distinção de maiúsculas) sobre o nome da
                  competição importada. {meta.fallbackText}
                </p>
              </CardHeader>
              <CardContent>
                {cfg.titleWeights.length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    Sem competições continentais configuradas. Adiciona pesos em "Pesos de títulos
                    continentais" primeiro.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {cfg.titleWeights.map((t) => {
                      ...
                      return (
                        <label
                          key={t.match}
                          className="flex items-center gap-2 text-sm cursor-pointer rounded border px-2 py-1.5 hover:bg-muted/40"
                        >
                          <Checkbox
                            checked={selected}
                            onCheckedChange={(v) =>
                              upd((c) => {
                                ...
                                if (v) list.add(t.match);
                                else list.delete(t.match);
                ...
                <div className="mt-3 flex gap-2">
                  <Button
                    ...
                    onClick={() =>
                      upd((c) => {
                        (c[meta.field] as string[]) = cfg.titleWeights.map((t) => t.match);
                      })
                    }
                  >
                    Selecionar todas
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Multiplicadores aplicados aos pontos base de cada fase em competições continentais e
              de seleções.
            </p>
          </CardHeader>
          <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <NumField
              label="Finalista vencido (×)"
              step={0.01}
              value={cfg.stageMultipliers.finalist}
              onChange={(v) =>
                upd((c) => {
                  c.stageMultipliers.finalist = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Meia-final (×)"
              step={0.01}
              value={cfg.stageMultipliers.semi}
              onChange={(v) =>
                upd((c) => {
                  c.stageMultipliers.semi = Math.max(0, v);
                })
              }
            />
            <NumField
              ...
              step={0.01}
              value={cfg.stageMultipliers.quarter}
              onChange={(v) =>
                upd((c) => {
                  c.stageMultipliers.quarter = Math.max(0, v);
                })
              }
            />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Multiplicador aplicado à pontuação consoante a antiguidade da época (1.00 = sem
              decaimento).
            </p>
          </CardHeader>
          <CardContent className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <NumField
              label="Última época (×)"
              step={0.01}
              value={cfg.decayMultipliers.last}
              onChange={(v) =>
                upd((c) => {
                  c.decayMultipliers.last = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Há 1 época (×)"
              step={0.01}
              value={cfg.decayMultipliers.age1}
              onChange={(v) =>
                upd((c) => {
                  c.decayMultipliers.age1 = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Há 2 épocas (×)"
              step={0.01}
              value={cfg.decayMultipliers.age2}
              onChange={(v) =>
                upd((c) => {
                  c.decayMultipliers.age2 = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Há 3 épocas (×)"
              step={0.01}
              value={cfg.decayMultipliers.age3}
              onChange={(v) =>
                upd((c) => {
                  c.decayMultipliers.age3 = Math.max(0, v);
                })
              }
            />
            <NumField
              label="Épocas mais antigas (×)"
              step={0.01}
              value={cfg.decayMultipliers.older}
              onChange={(v) =>
                upd((c) => {
                  c.decayMultipliers.older = Math.max(0, v);
                })
              }
            />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Quando ativo, os pontos da liga (coluna <em>Pnts</em>) são divididos pelo número de
              jogos disputados (coluna <em>Jgs</em>) antes de serem somados ao ranking — útil para
              comparar épocas com calendários diferentes. Aplica-se a Super League e Ligas
              Nacionais.
            </p>
          </CardHeader>
          <CardContent className="flex items-center gap-3">
            <input
              ...
              onChange={(e) =>
                upd((c) => {
                  c.normalizePointsByGames = e.target.checked;
                })
              }
            />
      ...
      <Accordion type="multiple" className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
                {sortNumKeys(divisions, cfg.divisionWeights, sortDiv).map((d) => (
                  <NumField
                    ...
                    label={`Div. ${d}`}
                    step={0.01}
                    value={cfg.divisionWeights[d] ?? 1}
                    onChange={(v) =>
                      upd((c) => {
                        c.divisionWeights[d] = v;
                      })
                    }
                  />
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
                {sortNumKeys(positions, cfg.positionPoints, sortPos).map((p) => (
                  <NumField
                    key={p}
                    label={`${p}.º lugar`}
                    value={cfg.positionPoints[p] ?? 0}
                    onChange={(v) =>
                      upd((c) => {
                        c.positionPoints[p] = v;
                      })
                    }
                  />
                ))}
        ...
        <AccordionItem value="titles" className="border rounded-lg px-4">
          ...
          <AccordionContent>
            <div className="space-y-3 pb-2">
              <p className="text-xs text-muted-foreground">
                Peso base atribuído a cada competição continental. O nome é comparado (sem
                acentos/maiúsculas) com a coluna <em>Competição</em> dos dados continentais. A
                pontuação final = peso do título × peso Continental × decaimento por época.
              ...
              <Button
                ...
                onClick={() =>
                  upd((c) => {
                    c.titleWeights.push({ match: "", label: "Nova competição", weight: 150 });
                  })
                }
              >
        ...
        <AccordionItem value="natleagues" className="border rounded-lg px-4">
          ...
          <AccordionContent>
            <div className="space-y-3 pb-2">
              <p className="text-xs text-muted-foreground">
                Multiplicador aplicado a pontos de posição, Pnts da liga e bónus de campeão das
                Ligas Nacionais. O nome é comparado (sem acentos/maiúsculas) com a coluna{" "}
                <em>Liga/Divisão</em> das classificações nacionais. Ligas sem entrada nesta lista
                usam multiplicador 1.
              </p>
                    ...
                    <div key={i} className="rounded-lg border border-border p-2 space-y-2">
                      <div className="flex items-end gap-2">
                        ...
                        <div className="w-24">
                          <NumField
                            ...
                            onChange={(v) =>
                              upd((c) => {
                                c.nationalLeagueWeights[i].weight = v;
                              })
                            }
                          />
                        </div>
                        <Button
                          ...
                          onClick={() =>
                            upd((c) => {
                              c.nationalLeagueWeights.splice(i, 1);
                            })
                          }
                        >
                          <Trash2 className="size-4 text-destructive" />
                      ...
                      <div className="rounded-md bg-muted/30 p-2 space-y-2">
                        ...
                        {bonusEntries.length === 0 ? (
                          <p className="text-[11px] text-muted-foreground">
                            Sem bónus extra por posição. Adiciona uma posição para premiar (ex.: 1º
                            +500, 2º +300).
                          </p>
                        ) : (
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                            {bonusEntries.map(([pos, val]) => (
                              <div
                                key={pos}
                                className="flex items-end gap-1 rounded border border-border/60 bg-background p-1.5"
                              >
              ...
              <Button
                ...
                onClick={() =>
                  upd((c) => {
                    c.nationalLeagueWeights.push({ match: "", label: "Nova liga", weight: 1 });
                  })
                }
              >
          ...
          <AccordionContent>
            <div className="space-y-3 pb-2">
              <p className="text-xs text-muted-foreground">
                ...
                Competições sem entrada nesta lista usam peso padrão 150. Os pontos brutos escalam
                com o peso da competição: vencedor = peso, finalista = peso × 0.25, meia-final =
                peso × 0.125, quartos = peso × 0.06. O ponderado multiplica ainda por pesoComp ×
                multFase × decaimento.
              </p>
                    ...
                    >
                      ...
                      <div className="w-24">
                        <NumField
                          ...
                          onChange={(v) =>
                            upd((c) => {
                              c.internationalWeights[i].weight = v;
                            })
                          }
                        />
                      </div>
                      <Button
                        ...
                        onClick={() =>
                          upd((c) => {
                            c.internationalWeights.splice(i, 1);
                          })
                        }
                      >
                        <Trash2 className="size-4 text-destructive" />
      ...
      <div className="space-y-3">
        <h2 className="text-lg font-semibold tracking-tight flex items-center gap-2">
          <AlertTriangle className="size-5 text-primary" /> Administração da Plataforma
        </h2>
        <p className="text-sm text-muted-foreground">
          Ferramentas de administração para exportação global configurável e reset total do universo
          FM. Não afetam Score Engine, Ranking Engine, Intelligence Engine, Imports, Dictionary nem
          Studios.
        </p>
        <AdminPanel />
      </div>
