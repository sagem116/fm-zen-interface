...
export const Route = createFileRoute("/super-league/treinador-campeoes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Histórico de títulos e promoções por treinador na Super League.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  const rows = useMemo(
    () => (data ? computeCoachChampions(data.data.standings, data.data.coaches) : []),
    [data],
  );
  ...
  if (!rows.length)
    return (
      <p className="text-muted-foreground">
        Sem dados da Super League. Importe uma época primeiro.
      </p>
    );
