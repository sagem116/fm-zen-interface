...
export const Route = createFileRoute("/desafios")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Cria desafios personalizados com pontos extra para clubes/treinadores/seleções.",
      },
    ],
  }),
...
function DesafiosPage() {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `desafios-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const replace = confirm(
        "Carregar OK. Substituir os desafios atuais?\nOK = substituir, Cancelar = juntar aos existentes.",
      );
      const next = replace
        ? incoming
        : [...list, ...incoming.map((d) => ({ ...d, id: crypto.randomUUID() }))];
      update(next);
  ...
  const filteredResults = useMemo(() => {
    ...
    return results.filter(({ desafio }) => {
      ...
      if (filterReqType !== "any" && !desafio.requirements.some((r) => r.type === filterReqType))
        return false;
      if (filterConsecutive && !desafio.requirements.some((r) => r.consecutive)) return false;
      if (q) {
        const hay =
          `${desafio.name} ${desafio.description ?? ""} ${desafio.requirements.map((r) => r.type + " " + r.match).join(" ")}`.toLowerCase();
  ...
  const linkFor = (subject: Subject) =>
    subject === "clubs"
      ? ("/clubes/$name" as const)
      : subject === "coaches"
        ? ("/treinadores/$name" as const)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Cria desafios personalizados. Os pontos extra são{" "}
            <strong>somados aos rankings mundiais</strong> de cada entidade que cumpre os
            requisitos.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => setWizardOpen(true)}>
            <Wand2 className="size-4" /> Assistente
          </Button>
          <Button onClick={newDesafio}>
            <Plus className="size-4" /> Novo
          </Button>
        </div>
      </div>

      <WizardDialog open={wizardOpen} onOpenChange={setWizardOpen} onCreate={addDesafios} />
        ...
        <Card>
          <CardContent className="py-3 flex flex-wrap items-end gap-3">
            <div className="flex-1 min-w-[200px] space-y-1">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Nome, descrição, competição…"
                  className="h-9 pl-8"
                />
              </div>
            </div>
            <div className="w-48 space-y-1">
              ...
              <Select
                value={filterSubject}
                onValueChange={(v) => setFilterSubject(v as Subject | "any")}
              >
            ...
            <div className="w-56 space-y-1">
              ...
              <Select
                value={filterReqType}
                onValueChange={(v) => setFilterReqType(v as ReqType | "any")}
              >
                <SelectTrigger className="h-9">
                  <SelectValue />
                </SelectTrigger>
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer pb-1.5">
              <Checkbox
                checked={filterConsecutive}
                onCheckedChange={(v) => setFilterConsecutive(!!v)}
              />
            ...
            {(search ||
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setFilterReqType("any");
                  setFilterConsecutive(false);
                }}
              >
                Limpar
              </Button>
          ...
          <CardContent className="py-12 text-center text-muted-foreground space-y-3">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              <Button onClick={() => setWizardOpen(true)}>
                <Wand2 className="size-4" /> Abrir Assistente
              </Button>
              <Button variant="outline" onClick={newDesafio}>
                <Plus className="size-4" /> Novo manual
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {filteredResults.map(({ desafio, matches }) => {
        ...
        const reqSummary = desafio.requirements
          .map((r) => {
            ...
            const cnt =
              r.count > 1
                ? ` ×${r.count}${r.consecutive ? " seguidas" : ""}`
                : r.consecutive
                  ? " seguidas"
                  : "";
          ...
          <Card key={desafio.id}>
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="ml-auto text-xs text-muted-foreground">
                      Bónus:{" "}
                      <span className="font-semibold text-foreground">{fmtPts(desafio.bonus)}</span>
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground truncate">
                    {reqSummary || "Sem requisitos"}
                  </p>
                  ...
                  <div className="grid sm:grid-cols-2 gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateDesafio(desafio.id, (d) => {
                            d.bonus = Number(e.target.value) || 0;
                          })
                        }
                    ...
                    <div className="flex flex-wrap gap-3">
                      {ALL_SUBJECTS.map((s) => (
                        <label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
                          <Checkbox
                            checked={desafio.subjects.includes(s)}
                            onCheckedChange={() => toggleSubject(desafio, s)}
                          />
                          {SUBJECT_LABEL[s]}
                        </label>
                      ))}
                  ...
                  <div className="space-y-2">
                      ...
                      <div key={idx} className="flex flex-wrap items-end gap-2 rounded border p-2">
                        <div className="w-60 space-y-1">
                          ...
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateDesafio(desafio.id, (d) => {
                                d.requirements[idx].type = v as ReqType;
                              })
                            }
                          >
                        ...
                        <div className="flex-1 min-w-[160px] space-y-1">
                          <Label className="text-[10px] text-muted-foreground">
                            {req.type === "hall-of-fame"
                              ? "(N/A — usa Top N)"
                              : req.type === "superleague-champion" ||
                                  req.type === "superleague-promotion"
                                ? "(qualquer SuperLeague)"
                                : req.type === "unbeaten-season" || req.type === "points-record"
                                  ? "Liga/divisão (substring; vazio = qualquer)"
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateDesafio(desafio.id, (d) => {
                                d.requirements[idx].match = e.target.value;
                              })
                            }
                            className="h-8"
                            placeholder="ex.: Champions / Premier / Div 1"
                          />
                        </div>
                        {(req.type === "unbeaten-season" || req.type === "points-record") && (
                          <div className="w-32 space-y-1">
                            ...
                            <Select
                              value={req.leagueScope ?? "any"}
                              onValueChange={(v) =>
                                updateDesafio(desafio.id, (d) => {
                                  d.requirements[idx].leagueScope = v as LeagueScope;
                                })
                              }
                            >
                              <SelectTrigger className="h-8">
                        ...
                        {req.type === "hall-of-fame" && (
                          <div className="w-20 space-y-1">
                            ...
                            <Input
                              type="number"
                              min={1}
                              value={req.hofTopN ?? 10}
                              onChange={(e) =>
                                updateDesafio(desafio.id, (d) => {
                                  d.requirements[idx].hofTopN = Math.max(
                                    1,
                        ...
                        <div className="w-20 space-y-1">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateDesafio(desafio.id, (d) => {
                                d.requirements[idx].count = Math.max(
                                  1,
                                  Number(e.target.value) || 1,
                                );
                              })
                        ...
                        <label className="flex items-center gap-1 text-xs cursor-pointer pb-1.5">
                          <Checkbox
                            checked={req.consecutive}
                            onCheckedChange={(v) =>
                              updateDesafio(desafio.id, (d) => {
                                d.requirements[idx].consecutive = !!v;
                              })
                            }
                          />
                          Seguidas
                    ...
                    {desafio.requirements.length === 0 && (
                      <p className="text-xs text-muted-foreground">
                        Sem requisitos — adiciona pelo menos um.
                      </p>
                    )}
                  </div>
                </div>
            ...
            {isOpen && (
              <CardContent className="space-y-3">
                    ...
                    <TableBody>
                      {matches.slice(0, 200).map((m) => (
                        <TableRow key={`${m.subject}|${m.entity}`}>
                          <TableCell>
                            <Badge variant="secondary">{SUBJECT_LABEL[m.subject]}</Badge>
                          </TableCell>
                          <TableCell className="font-medium">
                            <Link
                              to={linkFor(m.subject)}
                              params={{ name: m.entity }}
                ...
                {matches.length > 200 && (
                  <p className="text-xs text-muted-foreground">
                    A mostrar 200 de {matches.length}.
                  </p>
                )}
              </CardContent>
            )}
      ...
      {list.length > 0 && (
        <div className="text-xs text-muted-foreground flex items-center gap-2">
          <Save className="size-3" /> Os desafios são guardados automaticamente no browser e o bónus
          é aplicado aos rankings mundiais.
        </div>
      )}
    </div>
  );
...
function WizardDialog({
  ...
  const resetQuick = () => {
    ...
    setQSubjects(["clubs"]);
    setQSameYear(false);
    setQReqType("superleague-champion");
    setQMatch("");
    setQCount(1);
    setQConsec(false);
    setQHofTop(10);
  ...
  const addReqToQuick = () => {
    setQReqs((r) => [
      ...r,
      {
        ...
        leagueScope:
          qReqType === "unbeaten-season" || qReqType === "points-record" ? qScope : "any",
      },
    ]);
    setQMatch("");
    setQCount(1);
  ...
  const submitPresets = () => {
    ...
    if (!toAdd.length) {
      toast.error("Seleciona pelo menos um.");
      return;
    }
    onCreate(toAdd);
    setSelected({});
  ...
  const submitQuick = () => {
    if (!qName.trim()) {
      toast.error("Dá um nome ao desafio.");
      return;
    }
    const reqs = qReqs.length
      ? qReqs
      : [
          {
            type: qReqType,
    ...
    if (!reqs.length) {
      toast.error("Adiciona pelo menos um requisito.");
      return;
    }
    if (!qSubjects.length) {
      toast.error("Escolhe pelo menos um sujeito.");
      return;
    }
    const d: Desafio = {
      id: crypto.randomUUID(),
  ...
  const toggleQuickSubject = (s: Subject) => {
    setQSubjects((arr) =>
      arr.includes(s) ? (arr.length > 1 ? arr.filter((x) => x !== s) : arr) : [...arr, s],
    );
  };
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as "presets" | "quick")}>
          <TabsList>
            <TabsTrigger value="presets">
              <Sparkles className="size-4 mr-1" /> Sugestões
            </TabsTrigger>
            <TabsTrigger value="quick">
              <Plus className="size-4 mr-1" /> Criar rápido
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="presets" className="space-y-3 pt-2">
            <div className="flex items-center gap-2 justify-between">
              <p className="text-xs text-muted-foreground">
                {DESAFIO_PRESETS.length} sugestões disponíveis. Os IDs são regenerados ao adicionar.
              </p>
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {DESAFIO_PRESETS.map((p) => (
                <label
                  key={p.id}
                  className={`flex items-start gap-2 rounded border p-3 cursor-pointer hover:border-primary/50 ${selected[p.id] ? "border-primary bg-primary/5" : ""}`}
                >
                  <Checkbox
          ...
          <TabsContent value="quick" className="space-y-3 pt-2">
            <div className="grid sm:grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="h-9"
                  placeholder="ex.: Tri-campeão Continental"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  step="any"
                  value={qBonus}
                  onChange={(e) => setQBonus(Number(e.target.value) || 0)}
                  className="h-9 tabular-nums"
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-3">
                {ALL_SUBJECTS.map((s) => (
                  <label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox
                      checked={qSubjects.includes(s)}
                      onCheckedChange={() => toggleQuickSubject(s)}
                    />
                    {SUBJECT_LABEL[s]}
                  </label>
                ))}
            ...
            <Card>
              <CardHeader className="py-3">
                <CardTitle className="text-sm">Construtor de requisitos</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <div className="flex flex-wrap items-end gap-2">
                  ...
                  <div className="flex-1 min-w-[140px] space-y-1">
                    ...
                    <Input
                      ...
                      disabled={
                        ...
                        qReqType === "superleague-champion" ||
                        qReqType === "superleague-promotion"
                      }
                    />
                  </div>
                  ...
                  {qReqType === "hall-of-fame" && (
                    <div className="w-20 space-y-1">
                      ...
                      <Input
                        ...
                        min={1}
                        value={qHofTop}
                        onChange={(e) => setQHofTop(Math.max(1, Number(e.target.value) || 1))}
                        className="h-8 tabular-nums"
                      />
                    </div>
                  )}
                  ...
                  <Button size="sm" variant="outline" onClick={addReqToQuick}>
                    <Plus className="size-3" /> Adicionar
                  </Button>
                </div>
                {qReqs.length > 0 && (
                    ...
                    {qReqs.map((r, i) => (
                      <li key={i} className="flex items-center gap-2 rounded bg-muted/40 px-2 py-1">
                        <span className="flex-1">
                          {REQ_LABEL[r.type]}
                          {r.match ? ` "${r.match}"` : ""} ×{r.count}
                          {r.consecutive ? " seguidas" : ""}
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="size-6"
                          onClick={() => setQReqs((arr) => arr.filter((_, x) => x !== i))}
                        >
                          <Trash2 className="size-3 text-destructive" />
                        </Button>
                      </li>
                ...
                {qReqs.length === 0 && (
                  <p className="text-[11px] text-muted-foreground">
                    Se não adicionares, o requisito acima é usado diretamente.
                  </p>
                )}
              </CardContent>
            </Card>
