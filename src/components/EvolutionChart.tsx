...
const POS_COLORS = ["var(--chart-1)", "var(--chart-5)"];
...
export function EvolutionChart({
  ...
  const setMode = (m: ChartMode) => {
    onModeChange ? onModeChange(m) : setLocalMode(m);
  };
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <UITooltip>
            ...
            <TooltipContent className="max-w-xs text-xs leading-relaxed">
              ...
              <p>
                <span className="text-primary font-medium">Eixo esquerdo (área):</span> pontos{" "}
                {mode === "weighted"
                  ? "ponderados (com pesos de competição/divisão e desvalorização por época)"
                  : "brutos (sem pesos nem desvalorização)"}{" "}
                acumulados nesse ano.
              </p>
              <p className="mt-1">
                <span className="text-[color:var(--gold,#d4af37)] font-medium">
                  Eixo direito (linha):
                </span>{" "}
                posição ocupada no ranking mundial {mode} desse ano — quanto mais alto o ponto,
                melhor (eixo invertido).
              </p>
            </TooltipContent>
          </UITooltip>
        </div>
            ...
            {seriesList.map((s, i) => (
              <linearGradient key={i} id={`evoFill_${i}`} x1="0" y1="0" x2="0" y2="1">
                <stop
                  ...
                  stopOpacity={isComparison ? 0.18 : 0.35}
                />
                <stop
                  offset="100%"
                  stopColor={s.color ?? SERIES_COLORS[i % SERIES_COLORS.length]}
