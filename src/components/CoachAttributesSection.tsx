...
const ATTR_LABELS: Array<{
  key: keyof NonNullable<ReturnType<typeof useCoachFullData>["data"]>["coach"];
  label: string;
}> = [
...
export function CoachAttributesSection({ coachName }: { coachName: string }) {
  ...
  if (!data || (!c && !assignments.length)) return null;
        ...
        <CardContent>
          {c ? (
            ...
            <p className="text-sm text-muted-foreground">
              Sem características importadas para este treinador.
            </p>
          )}
