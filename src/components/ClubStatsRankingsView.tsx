      ...
      try {
        return (
          (typeof window !== "undefined"
            ? window.localStorage.getItem("fm-club-reputation-v1")
            : "") +
          "|" +
          (typeof window !== "undefined"
...
export function ClubStatsRankingsView({
  mode,
  withDecay,
}: {
  mode: "weighted" | "raw";
  ...
  const [density, setDensity] = useState<"comfy" | "compact">(() => {
    try {
      return typeof window !== "undefined" &&
        window.localStorage.getItem("fm-club-stats-density") === "compact"
        ? "compact"
        : "comfy";
    } catch {
      return "comfy";
  ...
  const setDensityPersist = (d: "comfy" | "compact") => {
    setDensity(d);
    try {
      window.localStorage.setItem("fm-club-stats-density", d);
    } catch {
      /* ignore */
    }
  };
  ...
  const years = useMemo(
    () => [...new Set(players.map((p) => p.season_year))].sort((a, b) => b - a),
    [players],
  );
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const p of filtered) {
      ...
      const w =
        mode === "weighted"
          ? compWeight(cfg.data.config, p) *
            decayFactor(cfg.data.config, p.season_year, latestYear, withDecay)
          : 1;
      let a = map.get(club);
      if (!a) {
        a = {
          club,
          country: clubCountry[club] ?? null,
  ...
  const selectedRows = useMemo(
    () => sorted.filter((r) => selected.has(r.club)),
    [sorted, selected],
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="flex rounded-md border border-border p-0.5" title="Densidade da tabela">
            <Button
              size="sm"
              variant={density === "comfy" ? "secondary" : "ghost"}
              className="h-7 px-2"
      ...
      <Card className="overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase">
              <tr>
                ...
                <th className="px-3 py-2 text-left">
                  {compFilter === "continental" || compFilter === "international"
                    ? "Continente"
                    : "País"}
                </th>
            ...
            <tbody>
              {pageRows.map((r, i) => (
                <tr
                  key={r.club}
                  className={`border-t border-border/50 hover:bg-muted/30 ${selected.has(r.club) ? "bg-primary/5" : ""}`}
                >
                  <td className={`${density === "compact" ? "px-2 py-1" : "px-2 py-2"}`}>
                    <Checkbox
                      checked={selected.has(r.club)}
                      onCheckedChange={() => toggleSelected(r.club)}
                      aria-label={`Selecionar ${r.club}`}
                    />
                  </td>
                  ...
                  <td className={`${cellPad} text-muted-foreground`}>
                    {r.competitions.length === 0 ? (
                      ...
                      <span title={r.competitions.join(", ")}>
                        {r.competitions.length} competições
                      </span>
                    )}
                  </td>
                    ...
                    <DrillCell<PlayerDrillRow>
                      ...
                      columns={[
                        ...
                        { key: "games", label: "Jogos", align: "right", value: (p) => p.games },
                        {
                          ...
                          label: "Golos",
                          align: "right",
                          value: (p) => fmtNum(p.gls, 2),
                        },
                        {
                          key: "ast",
                          label: "Assist.",
                          align: "right",
                          value: (p) => fmtNum(p.ast, 2),
                        },
                      ]}
                    />
                  </td>
                  ...
                  <td className={`${cellPad} text-right tabular-nums font-semibold`}>
                    {r.reputation == null ? "—" : fmtNum(r.reputation, 2)}
                  </td>
                </tr>
              ))}
              {pageRows.length === 0 && (
        ...
        <div className="flex items-center justify-between p-3 text-xs text-muted-foreground border-t border-border">
          <span>
            {sorted.length} clubes · página {page + 1}/{totalPages}
          </span>
                ...
                return (
                  <tr key={m.key} className="border-t border-border/50">
                    ...
                    {rows.map((r, i) => {
                      const v = vals[i];
                      const isBest =
                        best != null && typeof v === "number" && v === best && rows.length > 1;
                      return (
                        <td
                          key={r.club}
                          className={`px-3 py-2 text-right tabular-nums ${isBest ? "text-primary font-semibold" : ""}`}
                        >
                          {m.fmt(v)}
                        </td>
                      );
