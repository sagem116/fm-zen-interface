...
export function ChampionsTable({
  ...
  const Th = ({ k, label }: { k: ChampSort; label: string }) => (
    <th className="text-right p-3">
      <button
        onClick={() => setSort(k)}
        className={`hover:text-foreground ${sort === k ? "text-foreground" : ""}`}
      >
        {label}
      </button>
    </th>
            ...
            {sorted.map((r, i) => (
              <tr key={r.name} className="border-b border-border/50 hover:bg-muted/50">
                ...
                <td className="p-3 font-medium flex items-center gap-1">
                  {r.c > 0 && <Crown className="size-3 text-gold" />}
                  {r.name}
                </td>
                {showNac && <td className="p-3 text-muted-foreground">{r.nac ?? "—"}</td>}
                <td className="p-3 text-right">
                  <Num value={r.c} tip={r.tipC} />
                </td>
                <td className="p-3 text-right">
                  <Num value={r.p} tip={r.tipP} />
                </td>
                <td className="p-3 text-right">
                  <Num value={r.d} tip={r.tipD} />
                </td>
          ...
          <tbody>
            {sorted.map((r, i) => (
              <tr key={r.name} className="border-b border-border/50 hover:bg-muted/50">
                ...
                {showNac && <td className="p-3 text-muted-foreground">{r.nac ?? "—"}</td>}
                <td className="p-3 text-right">
                  <Num value={r.quaseSubida} tip={r.tipQS} />
                </td>
                <td className="p-3 text-right">
                  <Num value={r.quaseTitulo} tip={r.tipQT} />
                </td>
                <td className="p-3 text-right font-semibold tabular-nums">{r.total}</td>
              </tr>
            ))}
