...
import { trackRecentProfileFromPath } from "@/lib/profile/recent";
import { hydrateClubReputationsFromDb } from "@/lib/fm-club-reputation";
import { hydrateCompetitionReputationsFromDb } from "@/lib/fm-competition-reputation";
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        to: "/super-league/estatisticas-divisoes",
        label: "Estatísticas Divisões",
        icon: BarChart3,
      },
  ...
  },
  {
    ...
    items: [
      { to: "/recruitment-center", label: "Recruitment Center", icon: BriefcaseBusiness },
      { to: "/recruitment-center/dashboard", label: "Dashboard", icon: LayoutDashboard },
      { to: "/recruitment-center/pesquisa", label: "Pesquisa", icon: Search },
      { to: "/recruitment-center/tactical", label: "Tactical Recruitment", icon: Target },
      { to: "/recruitment-center/intelligence", label: "Recruitment Intelligence", icon: Brain },
      { to: "/recruitment-center/recomendacoes", label: "Recommendation Engine", icon: Sparkles },
      { to: "/recruitment-center/score", label: "Recruitment Score", icon: Sparkles },
      { to: "/recruitment-center/mercado", label: "Mercado", icon: Globe2 },
      { to: "/recruitment-center/observacoes", label: "Observações", icon: ClipboardList },
      { to: "/recruitment-center/shortlists", label: "Shortlists", icon: ListChecks },
      { to: "/recruitment-center/necessidades", label: "Necessidades", icon: Target },
      { to: "/recruitment-center/relatorios", label: "Scout Reports", icon: FileText },
      {
        to: "/recruitment-center/centro-comparacao",
        label: "Centro de Comparação",
        icon: GitCompareArrows,
      },
      { to: "/recruitment-center/historico", label: "Histórico", icon: HistoryIcon },
    ],
  },
  {
    title: "Ferramentas",
    items: [
      { to: "/minha-carreira", label: "Minha Carreira", icon: Crown },
      ...
      { to: "/scores-studio", label: "Score Studio", icon: Layers },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const { effectiveGroups, effectiveDebug } = useMemo(() => {
    ...
    for (const g of NAV_GROUPS)
      groupsMap.set(g.title, [
        ...g.items.filter((it) => (overrides[it.to] ?? g.title) === g.title),
      ]);
    const debugItems: NavItem[] = DEBUG_ITEMS.filter(
      (it) => (overrides[it.to] ?? "__debug__") === "__debug__",
    );
  ...
  useEffect(() => {
    trackRecentProfileFromPath(pathname);
  }, [pathname]);

  // Hydrate the reputation localStorage mirrors from Supabase on boot so
  // every synchronous consumer (Debug, rankings, dashboards, profiles) reads
  // exactly what's in `club_reputation_season` / `competition_reputation`.
  useEffect(() => {
    void hydrateClubReputationsFromDb();
    void hydrateCompetitionReputationsFromDb();
  }, []);
      ...
      <div className="flex min-h-screen bg-background text-foreground">
            ...
            {orderedGroups.map((group) => (
              <div key={group.title}>
                ...
                >
                  ...
                  <ChevronDown
                    className={cn("size-3 transition-transform", group.collapsed && "-rotate-90")}
                  />
                </button>
                {!group.collapsed && (
            ...
            {orderedDebug.length > 0 && (
              <div>
                ...
                >
                  <span>Debug</span>
                  <ChevronDown
                    className={cn("size-3 transition-transform", debugOpen && "rotate-180")}
                  />
                </button>
                {debugOpen && (
                  <div className="space-y-0.5">
            ...
            <nav className="md:hidden flex items-center gap-1 overflow-x-auto">
              {[...orderedGroups.flatMap((g) => g.items), ...orderedDebug].map((item) => (
                <Link
                  key={item.to}
                  to={item.to}
                  className="text-xs px-2 py-1 rounded text-muted-foreground hover:text-foreground whitespace-nowrap"
                  search={{ tab: undefined }}
                >
                  {item.label}
                </Link>
        ...
        <SidebarCustomizeDialog
          ...
          groups={NAV_GROUPS.map((g) => ({
            title: g.title,
            items: g.items.map((i) => ({ to: i.to, label: i.label })),
          }))}
