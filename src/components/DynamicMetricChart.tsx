...
function rowsFor(kind: ProfileKind, name: string, all: PlayerStatRow[]): PlayerStatRow[] {
  ...
  switch (kind) {
    case "player":
      return all.filter((r) => norm(r.player_name) === t);
    case "club":
      return all.filter((r) => norm(r.club) === t);
    case "competition":
      return all.filter((r) => norm(r.competition) === t);
    case "country":
      return all.filter((r) => norm(r.nationality) === t);
  }
}

export function DynamicMetricChart({
  kind,
  name,
  title,
}: {
  ...
  const series = useMemo(() => {
    ...
    for (const r of rows) {
      let v = m.pick(r);
      // Prefer Player Universe per-season individual values for certain keys when available
      if (kind === "player") {
        try {
          const entry = r.idu ? universe.getByIdu(r.idu) : universe.getByName(r.player_name);
          const hist = entry && r.season_year ? (entry.history?.[r.season_year] ?? null) : null;
          if (hist) {
            if (m.key === "age" && hist.season != null) v = (hist as any).age ?? v;
            if (m.key === "ca") v = (hist as any).ca ?? v;
            if (m.key === "cp") v = (hist as any).cp ?? v;
            if (m.key === "vp") v = (hist as any).value ?? v;
            if (m.key === "salary") v = (hist as any).salary ?? v;
          }
        } catch (e) {
          /* ignore */
        }
          ...
          <div style={{ width: "100%", height: 280 }}>
            <ResponsiveContainer>
              <LineChart data={series} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                ...
                <YAxis
                  tick={{ fontSize: 11 }}
                  tickFormatter={(v) => (m.money ? fmtMoney(Number(v)) : fmtNum(Number(v), 1))}
                  width={70}
                />
                <RTooltip
                  formatter={(v: number) => (m.money ? fmtMoney(Number(v)) : fmtNum(Number(v), 2))}
