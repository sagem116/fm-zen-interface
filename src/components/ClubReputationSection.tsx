  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <StatCard
          label="Reputação"
          value={current?.reputation == null ? "—" : fmtNum(current.reputation, 2)}
        />
        <StatCard
          ...
          value={
            current?.avg_attendance == null
              ? "—"
              : Math.round(current.avg_attendance).toLocaleString("pt-PT")
          }
        />
        <StatCard
          label="Detentores de Bilhetes de Época"
          value={
            current?.season_ticket_holders == null
              ? "—"
              : Math.round(current.season_ticket_holders).toLocaleString("pt-PT")
          }
        />
      </div>
