...
function aggregateByPlayer(rows: PRow[], stat: StatKey, agg: Agg) {
  const map = new Map<
    string,
    { name: string; value: number; club: string | null; season: number; competition: string }
  >();
  for (const r of rows) {
...
function RecordsTable({
  ...
  rows: {
    ...
    label: string;
    fmt: (n: number) => string;
    best: ReturnType<typeof aggregateByPlayer> | null;
  }[];
                  ...
                  <tr key={r.stat} className="border-b border-border/50 hover:bg-muted/50">
                    ...
                    {showCompetition && (
                      <td className="p-3">
                        <Link
                          to="/competicoes/$name"
                          params={{ name: r.best!.competition }}
                          className="hover:text-primary hover:underline"
                          search={{ tab: undefined }}
                        >
                          {r.best!.competition}
...
export function CompetitionRecordsSection({ competition }: { competition: string }) {
  ...
  const sections = useMemo(() => {
    ...
    const allTime = STATS.map((s) => ({
      stat: s.key,
      label: s.label,
      fmt: s.fmt,
      best: aggregateByPlayer(rows, s.key, s.agg),
    }));
    const bestSeason = STATS.map((s) => ({
      stat: s.key,
      label: s.label,
      fmt: s.fmt,
      best: bestSingleSeason(rows, s.key),
    }));
  ...
  return (
    <div className="space-y-6">
      <RecordsTable
        title="Recordes históricos (todas as épocas)"
        rows={sections.allTime}
        showClub
        showSeason
      />
      <RecordsTable
        title="Melhor época (registo único)"
        rows={sections.bestSeason}
        showClub
        showSeason
      />
    </div>
...
export function ClubRecordsSection({ clubName }: { clubName: string }) {
    ...
    const perCompetition = [...byCompetition.entries()]
      ...
      .map(([competition, rs]) => ({
        ...
        rows: STATS.map((s) => ({
          ...
          label: s.label,
          fmt: s.fmt,
          best: aggregateByPlayer(rs, s.key, s.agg),
        })),
      }));
  ...
  return (
    <div className="space-y-6">
      <RecordsTable
        title="Recordes do clube (todas as competições)"
        rows={sections.allTime}
        showSeason
        showCompetition
      />
      <RecordsTable
        title="Melhor época pelo clube (registo único)"
        rows={sections.bestSeason}
        showSeason
        showCompetition
      />
      {sections.perCompetition.map((c) => (
        <RecordsTable
          key={c.competition}
          title={`Recordes pelo clube — ${c.competition}`}
          rows={c.rows}
          showSeason
        />
      ))}
...
export function CountryRecordsSection({
  ...
  return (
    <div className="space-y-6">
      <RecordsTable
        title="Recordes do país (todas as competições)"
        rows={sections.allTime}
        showClub
        showSeason
        showCompetition
      />
      <RecordsTable
        title="Melhor época pelo país (registo único)"
        rows={sections.bestSeason}
        showClub
        showSeason
        showCompetition
      ...
      {sections.perCompetition.map((c) => (
        <RecordsTable
          key={c.competition}
          title={`Recordes do país — ${c.competition}`}
          rows={c.rows}
          showClub
          showSeason
        />
