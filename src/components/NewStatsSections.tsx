...
export function PlayerNewStatsSection({ playerName }: { playerName: string }) {
  ...
  const first = rows[0];
  const uni = first.idu ? universe.getByIdu(first.idu) : universe.getByName(first.player_name);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          ...
          {uni?.country && <Badge variant="outline">NAC: {uni.country}</Badge>}
          {uni?.idu && <Badge variant="outline">IDU: {uni.idu}</Badge>}
        </CardTitle>
      </CardHeader>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b border-border/50 hover:bg-muted/50">
                ...
                <td className="p-3 text-right tabular-nums">
                  {r.pass_pct == null ? "—" : fmtNum(r.pass_pct, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                ...
                <td className="p-3 text-right tabular-nums">
                  {r.shot_pct == null ? "—" : fmtNum(r.shot_pct, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {r.yellows == null ? "—" : fmtNum(r.yellows, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {r.reds == null ? "—" : fmtNum(r.reds, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
...
export function ClubNewStatsSection({
  clubName,
  season,
}: {
  clubName: string;
  ...
  const agg = useMemo(() => {
    ...
    const rowsByCompetition = [...byCompetition.values()]
      ...
      .sort(
        (a, b) =>
          Number(b.year) - Number(a.year) || a.competition.localeCompare(b.competition, "pt-PT"),
      );
      ...
      <CardContent className="p-0 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {agg.map((s) => (
              <tr
                key={`${s.year}-${s.compType}-${s.competition}`}
                className={`border-b border-border/50 hover:bg-muted/50 ${s.compType === "total" ? "border-t-2 bg-muted/30 font-semibold" : ""}`}
              >
                <td className="p-3 tabular-nums">{s.year === "total" ? "—" : s.year}</td>
                <td className="p-3 text-xs text-muted-foreground">
                  {s.compType === "total" ? "—" : COMP_LABEL[s.compType]}
                </td>
                <td className="p-3">
                  {s.compType === "total" ? (
                    "Total"
          ...
          <tbody>
            {stats.map((s) => (
              <tr key={s.season_year} className="border-b border-border/50 hover:bg-muted/50">
                ...
                <td className="p-3 text-right tabular-nums">
                  {s.xg_avg == null ? "—" : fmtNum(s.xg_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.pass_pct_avg == null ? "—" : fmtNum(s.pass_pct_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.tackles_per90_avg == null ? "—" : fmtNum(s.tackles_per90_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.fouls_per90_avg == null ? "—" : fmtNum(s.fouls_per90_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.shot_pct_avg == null ? "—" : fmtNum(s.shot_pct_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.yellows_avg == null ? "—" : fmtNum(s.yellows_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.reds_avg == null ? "—" : fmtNum(s.reds_avg, 2)}
                </td>
                <td className="p-3 text-right tabular-nums">
                  {s.avg_rating_avg == null ? "—" : fmtNum(s.avg_rating_avg, 2)}
                </td>
              </tr>
            ))}
          </tbody>
