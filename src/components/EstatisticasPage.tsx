    ...
    >
      <DialogContent className="max-w-5xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-auto">
          ...
          {state && state.rows.length > 1000 && (
            <p className="p-3 text-xs text-muted-foreground">
              A mostrar primeiros 1000 de {state.rows.length}.
            </p>
          )}
        </div>
      </DialogContent>
...
export function EstatisticasPage() {
  ...
  const years = useMemo(
    () => [...new Set(allPlayers.map((p) => p.season_year))].sort((a, b) => b - a),
    [allPlayers],
  );
  ...
  const competitions = useMemo(
    () =>
      uniqueSorted(
        allPlayers
          .filter((p) => compFilter === "unified" || p.comp_type === compFilter)
          .map((p) => p.competition),
      ),
    [allPlayers, compFilter],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return allPlayers.filter((p) => {
      ...
      if (q) {
        const hay = normText(
          `${p.player_name} ${p.club ?? ""} ${p.competition} ${p.country ?? ""} ${p.nationality ?? ""}`,
        );
  ...
  const compsTable = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      e.row.vp_total += r.vp || 0;
      e.row.sal_total += r.salary || 0;
      e.sAge += r.age || 0;
      e.sCA += r.ca || 0;
      e.sCP += r.cp || 0;
      e.sRA += r.ra || 0;
  ...
  const natPerCompetition = useMemo(() => {
    ...
    for (const [comp, inner] of map) {
      ...
      for (const [nat, n] of inner)
        rows.push({ competition: comp, nationality: nat, n, pct: (n / total) * 100 });
    }
    return rows;
  }, [filtered]);
  ...
  const byNationality = useMemo(() => {
    ...
    return [...map.values()].map((e) => ({
      ...
      n_players: e.players.size,
      vp_total: e.vp_total,
      vp_avg: avg(e.vp_total, e.n),
      sal_total: e.sal_total,
      sal_avg: avg(e.sal_total, e.n),
      age: avg(e.sAge, e.n),
      ca: avg(e.sCA, e.n),
      cp: avg(e.sCP, e.n),
    }));
  ...
  const byAge = useMemo(() => {
    const map = new Map<
      number,
      { age: number; n: number; sVP: number; sCA: number; sCP: number }
    >();
    for (const r of filtered) {
      if (!r.age) continue;
  ...
  const ageBuckets = useMemo(() => {
    ...
    return defs.map(([label, fn]) => ({
      bucket: label,
      n: filtered.filter((r) => r.age && fn(Math.floor(r.age))).length,
    }));
  }, [filtered]);
  ...
  const clubsByCountry = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      const s = m.get(r.country) ?? new Set<string>();
      s.add(r.club);
      m.set(r.country, s);
    }
  ...
  const clubsByCompetition = useMemo(() => {
    const m = new Map<string, Set<string>>();
    for (const r of filtered) {
      if (!r.club || !r.competition) continue;
      const s = m.get(r.competition) ?? new Set<string>();
      s.add(r.club);
      m.set(r.competition, s);
    }
  ...
  const clubsByContinent = useMemo(() => {
    ...
    for (const r of filtered) {
      ...
      const s = m.get(c) ?? new Set<string>();
      s.add(r.club);
      m.set(c, s);
    }
  ...
  if (!allPlayers.length) {
    return (
      <Card className="p-8 text-center text-sm text-muted-foreground">Sem dados importados.</Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Vista agregada e exploratória de toda a base de dados.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {COMP_TABS.map((t) => (
          <Button
            key={t.value}
            size="sm"
            variant={compFilter === t.value ? "secondary" : "outline"}
            onClick={() => {
              setCompFilter(t.value);
              setCompetition("");
            }}
          >
            {t.label}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div>
            ...
            <Input
              ...
              placeholder="jogador/clube/competição"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Select
              value={continent || "all"}
              onValueChange={(v) => setContinent(v === "all" ? "" : v)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setCompetition(v === "all" ? "" : v)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
      ...
      <Tabs defaultValue="competicoes">
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40">
                  <tr>
                    <Th
                      ...
                      label="Competição"
                      align="left"
                      current={sortComps.sortKey}
                      dir={sortComps.sortDir}
                      onClick={(k) => sortComps.toggle(k as string)}
                    />
                    <Th
                      k="clubs"
                      label="Clubes"
                ...
                <tbody>
                  {sortRows(compsTable, sortComps.sortKey, sortComps.sortDir).map((r) => (
                    <tr
                      key={`${r.comp_type}|${r.competition}`}
                      className="border-t border-border/40 hover:bg-muted/30 cursor-pointer"
                      onClick={() =>
                        drillBy(
                          (x) => x.competition === r.competition && x.comp_type === r.comp_type,
                          `Jogadores · ${r.competition}`,
                        )
                      }
                    >
                      <td className="px-3 py-1.5">
            ...
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {natPerCompetition
                    .sort(
                      (a, b) => a.competition.localeCompare(b.competition, "pt-PT") || b.n - a.n,
                    )
                    .map((r, i) => (
                      <tr
                        key={i}
                        ...
                        onClick={() =>
                          drillBy(
                            (x) =>
                              x.competition === r.competition &&
                              (x.nationality ?? "—") === r.nationality,
                            `${r.nationality} · ${r.competition}`,
                          )
                        }
                      >
                        <td className="px-3 py-1.5">
                ...
                <thead className="bg-muted/40">
                  <tr>
                    <Th
                      ...
                      label="Nacionalidade"
                      align="left"
                      current={sortNat.sortKey}
                      dir={sortNat.sortDir}
                      onClick={(k) => sortNat.toggle(k as string)}
                    />
                    <Th
                      k="n_players"
                      label="Jogadores"
                ...
                <tbody>
                  {sortRows(byNationality, sortNat.sortKey, sortNat.sortDir).map((r) => (
                    <tr
                      ...
                      onClick={() =>
                        drillBy(
                          (x) => (x.nationality ?? "—") === r.nationality,
                          `Jogadores · ${r.nationality}`,
                        )
                      }
                    >
                  ...
                  <thead className="bg-muted/40 sticky top-0">
                    <tr>
                      <Th
                        ...
                        label="Idade"
                        align="left"
                        current={sortAge.sortKey}
                        dir={sortAge.sortDir}
                        onClick={(k) => sortAge.toggle(k as string)}
                      />
                      <Th
                        k="n"
                        label="Jogadores"
                  ...
                  <tbody>
                    {sortRows(byAge, sortAge.sortKey, sortAge.sortDir).map((r) => (
                      <tr
                        ...
                        onClick={() =>
                          drillBy(
                            (x) => x.age != null && Math.floor(x.age) === r.age,
                            `Jogadores com ${r.age} anos`,
                          )
                        }
                      >
                  ...
                  {[...clubsByCountry]
                    ...
                    .map((r) => (
                      <tr
                        ...
                        className="border-t border-border/40 hover:bg-muted/30 cursor-pointer"
                        onClick={() =>
                          drillBy((x) => x.country === r.country, `Jogadores · ${r.country}`)
                        }
                      >
                        <td className="px-3 py-1.5">
                          <Link
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40">
                  <tr>
                    <th className="px-3 py-2 text-left uppercase text-xs">Continente</th>
                    <th className="px-3 py-2 text-right uppercase text-xs">Clubes</th>
                  </tr>
                </thead>
                <tbody>
                  {[...clubsByContinent]
                    ...
                    .map((r) => (
                      <tr
                        ...
                        onClick={() =>
                          drillBy(
                            (x) => (continentOf(x.country) ?? "—") === r.continent,
                            `Clubes · ${r.continent}`,
                          )
                        }
                      >
                ...
                <thead className="bg-muted/40">
                  <tr>
                    <Th
                      ...
                      label="Continente"
                      align="left"
                      current={sortCont.sortKey}
                      dir={sortCont.sortDir}
                      onClick={(k) => sortCont.toggle(k as string)}
                    />
                    <Th
                      k="comps"
                      label="Competições"
        ...
        <TabsContent value="dist">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="p-4">
              <h3 className="font-semibold mb-3">Distribuição C.A.</h3>
              <div style={{ width: "100%", height: 280 }}>
                <ResponsiveContainer>
                  <BarChart data={distCA}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
            ...
            <Card className="p-4">
              <h3 className="font-semibold mb-3">Distribuição Valor de Mercado</h3>
              <div style={{ width: "100%", height: 280 }}>
                <ResponsiveContainer>
                  <BarChart data={distVP}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                    <XAxis dataKey="label" tick={{ fontSize: 11 }} />
                    <YAxis />
                    <RTooltip />
            ...
            <Card className="p-4 lg:col-span-2">
              <h3 className="font-semibold mb-3">Distribuição Salarial</h3>
              <div style={{ width: "100%", height: 280 }}>
                <ResponsiveContainer>
                  <BarChart data={distSal}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                    <XAxis dataKey="label" tick={{ fontSize: 11 }} />
                    <YAxis />
                    <RTooltip />
            ...
            <div style={{ width: "100%", height: 320 }}>
              <ResponsiveContainer>
                <LineChart data={evoSeries}>
                  ...
                  <YAxis
                    tickFormatter={(v) =>
                      ["vp", "sal"].includes(evoMetric) ? fmtMoney(Number(v)) : fmtNum(Number(v), 1)
                    }
                    width={80}
                  />
                  <RTooltip
                    formatter={(v: number) =>
                      ["vp", "sal"].includes(evoMetric) ? fmtMoney(Number(v)) : fmtNum(Number(v), 2)
                    }
