...
export function RankingEvolutionSection({ kind, name, className }: Props) {
      ...
      kind === "club"
        ...
        : kind === "coach"
          ? mode === "weighted"
            ...
            : yearMaps.coachYearR
          : mode === "weighted"
            ? yearMaps.countryYearW
            : yearMaps.countryYearR;
  ...
  const stats = useMemo(() => {
    const valid = series.filter(
      (s): s is { year: number; rank: number } => typeof s.rank === "number",
    );
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Atual: <span className="font-medium text-foreground">#{stats.last}</span> · Melhor:
                #{stats.best} · Pior: #{stats.worst} · {stats.seasons} épocas
                {delta != null && delta !== 0 && (
                  <>
                    {" "}
                    ·{" "}
