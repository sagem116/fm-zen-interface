...
export function LeagueStatsPage({ scope, title, intro }: Props) {
  ...
  const filters: LeagueFilters = {
    ...
    caMin: numOrNull(caMin),
    caMax: numOrNull(caMax),
    vpMin: numOrNull(vpMin),
    vpMax: numOrNull(vpMax),
    salaryMin: numOrNull(salMin),
    salaryMax: numOrNull(salMax),
    ageMin: numOrNull(ageMin),
    ageMax: numOrNull(ageMax),
    search,
  };
  ...
  const tableRows = sorted.map((r, i) => {
    ...
    for (const c of def.columns)
      obj[c.label] = Number(
        formatVal(c.value(r, extras), c.fmt).replace(/\./g, "").replace(",", "."),
      );
    return obj;
  });
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-start gap-2 rounded-md border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
            ...
            <span>
              <strong className="text-foreground">{def.label}:</strong> {def.description}
            </span>
          </div>
        </CardContent>
      </Card>
                  ...
                  {def.columns
                    ...
                    .map((c) => (
                      <td key={c.key} className="p-3 text-right tabular-nums">
                        {formatVal(c.value(r, extras), c.fmt)}
                      </td>
                    ))}
