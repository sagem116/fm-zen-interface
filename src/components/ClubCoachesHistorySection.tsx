...
async function fetchClubCoachHistory(clubName: string): Promise<Row[]> {
  ...
  const [{ data: assigns }, { data: seasons }, { data: coaches }] = await Promise.all([
    sb
      ...
      .select("season_id, coach_name, club_role")
      .eq("club_name", clubName),
    sb.from("seasons").select("id, year"),
    sb.from("coaches").select("name, nationality"),
  ]);
  const seasonMap = new Map<string, number>();
  for (const s of (seasons ?? []) as Array<{ id: string; year: number }>)
    seasonMap.set(s.id, s.year);
  const natMap = new Map<string, string | null>();
  for (const c of (coaches ?? []) as Array<{ name: string; nationality: string | null }>)
    natMap.set(c.name, c.nationality);
  return (
    (assigns ?? []) as Array<{ season_id: string; coach_name: string; club_role: string | null }>
  ).map((a) => ({
...
export function ClubCoachesHistorySection({ clubName }: { clubName: string }) {
  ...
  const sorted = useMemo(
    () =>
      [...rows].sort(
        (a, b) =>
          b.season_year - a.season_year || a.coach_name.localeCompare(b.coach_name, "pt-PT"),
      ),
    [rows],
  );
