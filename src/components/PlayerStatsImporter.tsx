...
import {
  parsePlayerStatsWorkbook,
  type PlayerStatsParseResult,
} from "@/lib/fm-player-stats-parser";
import {
  importPlayerStats,
  logPlayerStatsImport,
  fetchAllPlayerStats,
} from "@/lib/fm-player-stats-db";
...
export function PlayerStatsImporter() {
  ...
  async function runImport() {
    ...
    try {
      ...
      try {
        const [all, sources] = await Promise.all([fetchAllPlayerStats(), fetchClubMapSources()]);
        ...
        for (const r of all) {
          ...
          if (r.comp_type === "superleague" && !r.competition)
            compsSemDivisao.add(r.competition || "?");
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Aceita um único Excel com as folhas <em>Divisão</em>, <em>Ligas Nacionais</em>,{" "}
          <em>Continental</em> e/ou <em>Internacional</em>. Folhas ausentes são ignoradas.
          Reimportar a mesma época substitui os dados anteriores das folhas presentes.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
              ...
              {Object.values(e.parse.bySheet).map((s) => (
                <div key={s.sheet} className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground">
                    folha "{s.sheet}" — {s.count} linhas
                  </span>
                </div>
              ))}
              {e.parse.skippedSheets.length > 0 && (
                <p className="text-muted-foreground italic">
                  Ignoradas: {e.parse.skippedSheets.join(", ")}
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <ImportProgressOverlay
          active={importing}
          label={`A importar estatísticas de jogadores (${entries.length} ficheiro${entries.length === 1 ? "" : "s"})`}
          historyKey="player-stats"
        />
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <Stat
                  ...
                  value={report.clubsSemCompeticao}
                  warn={report.clubsSemCompeticao > 0}
                />
                <Stat
                  label="Clubes duplicados"
                  value={report.clubsDuplicados}
                  warn={report.clubsDuplicados > 0}
                />
                <Stat
                  label="Comp. sem divisão"
                  value={report.competicoesSemDivisao}
                  warn={report.competicoesSemDivisao > 0}
                />
                <Stat
                  label="Países desconhecidos"
                  value={report.paisesNaoReconhecidos.length}
                  warn={report.paisesNaoReconhecidos.length > 0}
                />
              </div>
              {report.paisesNaoReconhecidos.length > 0 && (
                <div className="rounded-md border border-amber-500/40 bg-amber-500/5 p-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {report.paisesNaoReconhecidos.slice(0, 20).join(", ")}
                    {report.paisesNaoReconhecidos.length > 20 ? "…" : ""}
                  </p>
                </div>
              )}
              {report.warnings.length > 0 && (
                <details className="text-xs">
                  <summary className="cursor-pointer text-muted-foreground">
                    Avisos do parser ({report.warnings.length})
                  </summary>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button asChild>
              <Link to="/debug-mapeamento-clubes" search={{ tab: undefined }}>
                <Bug className="size-4 mr-2" /> Abrir Debug de Mapeamento
              </Link>
            </Button>
          </DialogFooter>
        </DialogContent>
...
function Stat({ label, value, warn }: { label: string; value: number; warn?: boolean }) {
  return (
    <div
      className={`rounded-lg border p-3 ${warn ? "border-amber-500/40 bg-amber-500/5" : "border-border"}`}
    >
