...
interface Props {
  ...
  /** Optional explain callback; when provided renders an "Explicar" chip on each row. */
  ...
  const inner = onClick ? (
    <button
      ...
      title={
        active
          ? dir === "asc"
            ? "Ordem crescente — clica para descer"
            : "Ordem decrescente — clica para subir"
          : "Ordenar"
      }
...
export function SeasonsRankTable({
  ...
  const {
    ...
    setSortKey,
    sortDir,
  } = useEntrySort(entries, mode, extrasMap, evolution);
  ...
  const to =
    kind === "clubes"
      ...
      : kind === "treinadores"
        ...
        : "/paises/$name";
  const label =
    nameLabel ?? (kind === "clubes" ? "Clube" : kind === "treinadores" ? "Treinador" : "País");
  ...
  const deltas = useMemo(() => {
    const out: Record<
      string,
      { ptsDelta: number; rankDelta: number | null; lastYear: number; prevYear: number } | null
    > = {};
    ...
    <Card className="mt-4 overflow-hidden">
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: cols, minWidth: "max-content" }}>
          ...
          >
            <HeaderCell className="text-left" pad={cellPad}>
              #
            </HeaderCell>
            <HeaderCell
              className="text-left sticky left-0 bg-card/95 backdrop-blur z-10"
              pad={cellPad}
            >
              {label}
            </HeaderCell>
            ...
            <HeaderCell className="text-right" pad={cellPad}>
              Δ vs anterior
            </HeaderCell>
            {years.map((y) => (
              <HeaderCell
          ...
          >
            {items.map((vi) => {
              ...
              return (
                ...
                >
                  <div
                    className={`${cellPad} font-bold ${i < 3 ? "text-gold" : "text-muted-foreground"}`}
                  >
                    {i + 1}
                  </div>
                  <div
                    className={`${cellPad} font-medium truncate sticky left-0 bg-background/95 backdrop-blur z-[1] flex items-center gap-1.5 min-w-0`}
                  >
                    <Link
                      to={to}
                      params={{ name: e.name }}
                    ...
                    {onExplain && (
                      <button
                        ...
                        aria-label={`Explicar ${e.name}`}
                        className="shrink-0 opacity-40 hover:opacity-100 hover:text-primary transition-opacity"
                      >
                  ...
                  {extraCols?.map((ec) => {
                    ...
                    const inner =
                      label !== undefined ? (
                        <span
                          className={`truncate inline-block max-w-full align-bottom ${!label ? "text-muted-foreground/30" : ""} ${tip ? "underline decoration-dotted cursor-help" : ""}`}
                        >
                          {label || "—"}
                        </span>
                      ) : (
                        <span
                          className={`tabular-nums ${v ? "" : "text-muted-foreground/30"} ${tip && v ? "underline decoration-dotted cursor-help" : ""}`}
                        >
                          {v || "—"}
                        </span>
                      );
                  ...
                  <div className={`${cellPad} text-right`}>
                    {d ? (
                      <DeltaCell ptsDelta={d.ptsDelta} rankDelta={d.rankDelta} />
                    ) : (
                  ...
                  {years.map((y) => {
                    ...
                    const cell = (
                      <div
                        className={`${cellPad} text-right tabular-nums ${v ? "" : "text-muted-foreground/30"}`}
                      >
                        {v ? fmtPts(v) : "—"}
                      </div>
                    );
                    ...
                    type Line = {
                      ...
                      raw: number;
                      weighted: number;
                      mult?: { compW: number; divW: number; decay: number };
                    };
                    ...
                    return (
                      <Tooltip key={y}>
                        <TooltipTrigger asChild>
                          <div
                            className={`${cellPad} text-right tabular-nums cursor-help ${v ? "" : "text-muted-foreground/30"}`}
                          >
                            {fmtPts(v)}
                          </div>
                        </TooltipTrigger>
                              ...
                              <div key={idx}>
                                ...
                                {l.mult && (
                                  <div className="text-muted-foreground/80 text-[10px] pl-2">
                                    Peso comp ×{fmtPts(l.mult.compW)} · Div ×{fmtPts(l.mult.divW)} ·
                                    Decay ×{fmtPts(l.mult.decay)}
                                  </div>
                                )}
                              </div>
                            ))}
