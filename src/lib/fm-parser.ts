...
function parseVP(v: unknown): number {
  ...
  if (s.endsWith("M")) {
    mult = 1_000_000;
    s = s.slice(0, -1);
  } else if (s.endsWith("m") || s.endsWith("k") || s.endsWith("K")) {
    mult = 1_000;
    s = s.slice(0, -1);
  }
  s = s.replace(/,/g, ".");
...
export function detectKind(wb: XLSX.WorkBook): "superleague" | "national" {
  ...
  if (names.some((n) => n.includes("equipas_pais") || n.includes("pesos_fixos")))
    return "superleague";
  if (names.some((n) => n.includes("ligas nacionais") || n.includes("continenta")))
    return "national";
  ...
  if (
    names.some(
      (n) =>
        ...
        n === "superleague" ||
        n === "superleague1" ||
        n.startsWith("divisao") ||
        n.startsWith("superleague"),
    )
  )
    return "superleague";
  return "national";
...
export function parseWorkbook(buffer: ArrayBuffer, filename = ""): ParseResult {
  ...
  if (ep && ep.length) {
    ...
    if (clubCol) {
      for (const r of ep) {
        ...
        if (!club) continue;
        data.teamCountry.push({
          club,
          country: countryCol ? String(r[countryCol] ?? "").trim() || null : null,
        });
      }
    }
  }
  ...
  if (pf && pf.length) {
    ...
    messages.push({
      level: "yellow",
      text: "⚠ Folha 'Pesos_Fixos' não encontrada — serão usados pesos padrão",
    });
  }
  ...
  if (kind === "superleague") {
    ...
    if (!rk || !rk.length) {
      messages.push({
        level: "red",
        text: "✖ Folha de classificação da Super League não encontrada (aceita: Ranking, Divisão, Division, Super League, SuperLeague, ...)",
      });
    } else {
  ...
  if (kind === "national") {
    ...
    if (!ln || !ln.length) {
      messages.push({
        level: "red",
        text: "✖ Folha obrigatória 'Ligas Nacionais' inexistente ou vazia",
      });
    } else {
    ...
    if (cc && cc.length) {
      ...
      const sf1Col = findCol(cc[0], [
        "Meia Final Equipa 1",
        "Meia-Final Equipa 1",
        "Meias Finais Equipa 1",
        "Semi Final Equipa 1",
        "SF Equipa 1",
        "MF Equipa 1",
      ]);
      const sf2Col = findCol(cc[0], [
        "Meia Final Equipa 2",
        "Meia-Final Equipa 2",
        "Meias Finais Equipa 2",
        "Semi Final Equipa 2",
        "SF Equipa 2",
        "MF Equipa 2",
      ]);
      const qf1Col = findCol(cc[0], [
        "Quartos de Final Equipa 1",
        "Quartos Final Equipa 1",
        "QF Equipa 1",
      ]);
      const qf2Col = findCol(cc[0], [
        "Quartos de Final Equipa 2",
        "Quartos Final Equipa 2",
        "QF Equipa 2",
      ]);
      const qf3Col = findCol(cc[0], [
        "Quartos de Final Equipa 3",
        "Quartos Final Equipa 3",
        "QF Equipa 3",
      ]);
      const qf4Col = findCol(cc[0], [
        "Quartos de Final Equipa 4",
        "Quartos Final Equipa 4",
        "QF Equipa 4",
      ]);
    ...
    const ciName = wb.SheetNames.find((s) => {
      const n = norm(s);
      return (
        n.includes("sele") && (n.includes("compt") || n.includes("comp") || n.includes("internac"))
      );
    });
    ...
    if (ci && ci.length) {
      ...
      const sf1CoachCol = findCol(ci[0], [
        "Treinador Meia Final 1",
        "Treinador MF 1",
        "Treinador SF 1",
        "Treinador Meia-Final 1",
        "Coach Meia Final 1",
      ]);
      const sf2Col = findCol(ci[0], [
        "Meia Final Equipa 2",
        "Meia-Final Equipa 2",
        "Meias Finais Equipa 2",
        "Semi Final Equipa 2",
      ...
      const sf2CoachCol = findCol(ci[0], [
        "Treinador Meia Final 2",
        "Treinador MF 2",
        "Treinador SF 2",
        "Treinador Meia-Final 2",
        "Coach Meia Final 2",
      ]);
      const qf1Col = findCol(ci[0], [
        "Quartos de Final Equipa 1",
        "Quartos Final Equipa 1",
        "QF Equipa 1",
      ]);
      const qf1CoachCol = findCol(ci[0], [
        "Quartos de Final Treinador 1",
        "Treinador Quartos de Final 1",
        "Treinador QF 1",
        "Quartos Final Treinador 1",
      ]);
      const qf2Col = findCol(ci[0], [
        "Quartos de Final Equipa 2",
        "Quartos Final Equipa 2",
        "QF Equipa 2",
      ]);
      const qf2CoachCol = findCol(ci[0], [
        "Quartos de Final Treinador 2",
        "Treinador Quartos de Final 2",
        "Treinador QF 2",
        "Quartos Final Treinador 2",
      ]);
      const qf3Col = findCol(ci[0], [
        "Quartos de Final Equipa 3",
        "Quartos Final Equipa 3",
        "QF Equipa 3",
      ]);
      const qf3CoachCol = findCol(ci[0], [
        "Quartos de Final Treinador 3",
        "Treinador Quartos de Final 3",
        "Treinador QF 3",
        "Quartos Final Treinador 3",
      ]);
      const qf4Col = findCol(ci[0], [
        "Quartos de Final Equipa 4",
        "Quartos Final Equipa 4",
        "QF Equipa 4",
      ]);
      const qf4CoachCol = findCol(ci[0], [
        "Quartos de Final Treinador 4",
        "Treinador Quartos de Final 4",
        "Treinador QF 4",
        "Quartos Final Treinador 4",
      ]);
      ...
      messages.push({
        level: "yellow",
        text: `⚠ Folha '${ciName}' encontrada, mas sem linhas de jogos para importar`,
      });
    } else {
      messages.push({
        level: "yellow",
        text: "⚠ Folha 'Compts Seleções' não encontrada — competições internacionais ficam vazias",
      });
    }
  }
  ...
  if (tr && tr.length) {
    ...
    messages.push({
      level: "yellow",
      text: "⚠ Folha 'Treinadores' não encontrada — clubes ficam sem treinador associado",
    });
  }
  ...
  // --- Jogadores (superleague + national) ---
  {
    ...
    if (jg && jg.length) {
      ...
      messages.push({
        level: "yellow",
        text: "⚠ Folha 'Jogadores' não encontrada — páginas de jogadores ficam vazias para esta época",
      });
    }
  }
