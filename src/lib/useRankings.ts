...
import { ensureAllData } from "./useAllData";
import {
  computeRankings,
  computeInternationalRankings,
  type RankingEntry,
  type BreakdownItem,
  type ComputeResult,
} from "./fm-rankings";
...
function mergeInternationalIntoRanks(
  ranks: ComputeResult,
  intl: ReturnType<typeof computeInternationalRankings>,
): ComputeResult {
  return {
    ...ranks,
...
export function useRankings() {
  ...
  return useQuery({
    queryKey: ["fm-rankings", "v3"],
    queryFn: async () => {
      const [data, cfg] = await Promise.all([ensureAllData(qc), fetchActiveConfig()]);
...
export function useRankingsNoDecay() {
  const qc = useQueryClient();
  return useQuery({
    queryKey: ["fm-rankings", "v3", "no-decay"],
    queryFn: async () => {
      const [data, cfg] = await Promise.all([ensureAllData(qc), fetchActiveConfig()]);
      const flatConfig = {
        ...cfg.config,
