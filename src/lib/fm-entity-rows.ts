...
} from "./fm-competition-reputation";
  ...
  for (const r of rows) {
    ...
    if (!e) {
      e = {
        ...
        country: r.country,
        comp: r.competition,
        continent: r.continent ?? continentOf(r.country ?? ""),
        n: 0,
        vp: 0,
        sal: 0,
        ca: 0,
...
export function buildCompetitionRows(rows: PlayerStatRow[]): EntityRow[] {
  ...
  return [...map.values()].map((e, i) => {
    const avgRep = e.clubReps.length
      ? e.clubReps.reduce((a, b) => a + b, 0) / e.clubReps.length
      : 0;
    return {
      id: `comp-${i}-${e.name}`,
      name: e.name,
...
export function buildCountryRows(rows: PlayerStatRow[]): EntityRow[] {
  ...
  for (const r of rows) {
    ...
    let e = map.get(k);
    if (!e) {
      e = { name: k, continent: continentOf(k), n: 0, vp: 0, sal: 0, clubs: new Set() };
      map.set(k, e);
    }
    e.n++;
    e.vp += r.vp;
...
export function buildRows(kind: EntityKind, rows: PlayerStatRow[]): EntityRow[] {
  switch (kind) {
    case "jogador":
      return buildPlayerRows(rows);
    case "clube":
      return buildClubRows(rows);
    case "competicao":
      return buildCompetitionRows(rows);
    case "pais":
      return buildCountryRows(rows);
...
export function metaFieldsFor(kind: EntityKind): { key: string; label: string }[] {
  ...
  if (kind === "clube")
    return [
      ...
      { key: "COMPETICAO", label: "Competição" },
      { key: "CONTINENTE", label: "Continente" },
      { key: "CLUBE", label: "Nome" },
    ];
  if (kind === "competicao")
    return [
      { key: "PAIS", label: "País" },
      { key: "CONTINENTE", label: "Continente" },
      { key: "COMP_TYPE", label: "Tipo" },
      { key: "COMPETICAO", label: "Nome" },
    ];
  if (kind === "treinador")
    return [
      { key: "NAC", label: "Nacionalidade" },
      { key: "CLUBE", label: "Último Clube" },
