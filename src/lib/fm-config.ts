...
export interface FmConfig {
  ...
  nationalLeagueWeights: {
    match: string;
    label: string;
    weight: number;
    positionBonuses?: Record<number, number>;
  }[];
...
export function cloneConfig(c: FmConfig): FmConfig {
  return {
    ...
    nationalLeagueWeights: c.nationalLeagueWeights.map((t) => ({
      ...t,
      positionBonuses: t.positionBonuses ? { ...t.positionBonuses } : undefined,
    })),
...
export function cfgTitleWeight(
  cfg: FmConfig,
  competition: string,
...
export function cfgNationalLeagueWeight(
  cfg: FmConfig,
...
export function configToRows(profileId: string, cfg: FmConfig): ConfigRow[] {
  ...
  for (const [k, v] of Object.entries(cfg.positionPoints))
    rows.push({ profile_id: profileId, category: "position", key: k, value: v });
  for (const [k, v] of Object.entries(cfg.divisionWeights))
    rows.push({ profile_id: profileId, category: "division", key: k, value: v });
  for (const [k, v] of Object.entries(cfg.competitionWeights))
    rows.push({ profile_id: profileId, category: "competition", key: k, value: v });
  for (const t of cfg.titleWeights)
    rows.push({
  ...
  for (const t of cfg.nationalLeagueWeights) {
    ...
    if (t.positionBonuses) {
      for (const [pos, val] of Object.entries(t.positionBonuses)) {
        ...
        rows.push({
          profile_id: profileId,
          category: "national-league-pos-bonus",
          key: `${t.match}\u0001${pos}`,
          value: n,
        });
      }
  ...
  rows.push({
    profile_id: profileId,
    category: "meta",
    key: "normalizePointsByGames",
    value: cfg.normalizePointsByGames ? 1 : 0,
  });
  return rows;
}
  ...
  for (const r of rows) {
    ...
    switch (r.category) {
      ...
      case "division":
        ...
        break;
      case "competition":
        if (r.key in cfg.competitionWeights)
          (cfg.competitionWeights as Record<string, number>)[r.key] = v;
        break;
      case "title": {
      ...
      case "national-league": {
        const [match, label] = r.key.split("\u0001");
        cfg.nationalLeagueWeights.push({
          match: match ?? "",
          label: label ?? match ?? "",
          weight: v,
        });
      ...
      case "meta":
        ...
        break;
      case "decay":
        if (r.key in cfg.decayMultipliers)
          (cfg.decayMultipliers as unknown as Record<string, number>)[r.key] = v;
        break;
      case "stage":
        if (r.key in cfg.stageMultipliers)
          (cfg.stageMultipliers as unknown as Record<string, number>)[r.key] = v;
        break;
    }
  }
