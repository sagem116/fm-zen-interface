...
export function filterPlayerRows(
  ...
  return rows.filter((r) => {
    ...
    if (q) {
      const hay = normText(
        `${r.player_name} ${r.club ?? ""} ${r.competition} ${r.country ?? ""} ${r.nationality ?? ""}`,
      );
    ...
    return rows
      .map((r) => {
        ...
        const w =
          mode === "weighted"
            ? raw *
              compWeight(ctx.config, r) *
              decayFactor(ctx.config, r.season_year, ctx.latestYear, ctx.withDecay)
            : raw;
        return {
...
export function rankCompetitions(
  ...
  const map = new Map<
    string,
    { row: CompetitionRankRow; totals: Record<string, number>; weightSum: number }
  >();
