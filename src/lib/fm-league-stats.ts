...
function leagueKeyForPlayer(
  p: PlayerRow,
  standDivByClubSeason: Map<string, number>,
  scope: Scope,
): string | null {
  if (scope === "superleague") {
...
export const RANKINGS: RankingDef[] = [
  {
    ...
    description:
      "Composto que combina CA, valor de mercado, salário, CP e nº de jogadores (35/25/20/10/10).",
    sortKey: "score",
    sortDir: "desc",
    columns: [
      {
    ...
    extras: (rows) => {
      ...
      for (const r of rows) {
        score[r.league] =
          0.35 * (nCa.get(r.league) ?? 0) +
          0.25 * (nVp.get(r.league) ?? 0) +
          0.2 * (nSal.get(r.league) ?? 0) +
          0.1 * (nCp.get(r.league) ?? 0) +
          0.1 * (nN.get(r.league) ?? 0);
      }
      return { score };
    },
  },
  {
    ...
    description: "Ordenado pelo CA médio dos jogadores da liga — talento atual no terreno.",
    sortKey: "caAvg",
    sortDir: "desc",
    columns: [
      { key: "caAvg", label: "CA Médio", value: (r) => r.caAvg, fmt: "dec1" },
  ...
  },
  {
    ...
    description: "Ordenado pelo CP médio — potencial futuro dos jogadores em desenvolvimento.",
    sortKey: "cpAvg",
    sortDir: "desc",
    columns: [
      { key: "cpAvg", label: "CP Médio", value: (r) => r.cpAvg, fmt: "dec1" },
  ...
  {
    key: "financial",
    label: "Poder Financeiro",
    description: "Composto pelo valor de mercado (60%) e salário médio (40%) — poderio económico.",
    sortKey: "score",
    sortDir: "desc",
    columns: [
      {
    ...
    extras: (rows) => {
      ...
      const score: Record<string, number> = {};
      for (const r of rows)
        score[r.league] = 0.6 * (nV.get(r.league) ?? 0) + 0.4 * (nS.get(r.league) ?? 0);
      return { score };
    },
  },
  {
    key: "youngest",
    label: "Mais Jovem",
    description: "Ligas com a idade média mais baixa — apostas no futuro.",
    sortKey: "ageAvg",
    sortDir: "asc",
  ...
  },
  {
    ...
    description: "Ligas com a idade média mais alta — experiência acumulada.",
    sortKey: "ageAvg",
    sortDir: "desc",
    columns: [{ key: "ageAvg", label: "Idade Média", value: (r) => r.ageAvg, fmt: "dec1" }],
  },
  {
    key: "offensive",
    label: "Mais Ofensiva",
    description: "Mais golos marcados por jogo — ataque produtivo.",
    sortKey: "gmPerGame",
    sortDir: "desc",
    columns: [
  ...
  {
    key: "defensive",
    label: "Mais Defensiva",
    description:
      "Menos golos marcados por jogo (do ponto de vista coletivo da liga) — fraco poder ofensivo.",
    sortKey: "gmPerGame",
    sortDir: "asc",
    columns: [
      { key: "gmTotal", label: "GM", value: (r) => r.gmTotal, fmt: "int" },
  ...
  {
    key: "spectacle",
    label: "Mais Espetacular",
    description: "Soma de golos marcados+sofridos por jogo — jogos mais espetaculares e abertos.",
    sortKey: "spectaclePerGame",
    sortDir: "desc",
    columns: [
      { key: "gmTotal", label: "GM", value: (r) => r.gmTotal, fmt: "int" },
      { key: "gsTotal", label: "GS", value: (r) => r.gsTotal, fmt: "int" },
      {
        key: "spectaclePerGame",
        label: "Golos Totais/Jogo",
        value: (r) => r.spectaclePerGame,
        fmt: "dec2",
      },
    ],
  },
  {
    ...
    description:
      "Quanto menor o desvio padrão dos pontos, mais equilibrada a liga (score 100 = máxima paridade).",
    sortKey: "score",
    sortDir: "desc",
    columns: [
      {
    ...
    extras: (rows) => ({
      score: toMap(rows, (r) => 100 - (normalize(rows, (x) => x.pointsStd).get(r.league) ?? 0)),
    }),
  },
  {
    key: "parityFin",
    label: "Paridade Financeira",
    description:
      "Quanto menor o desvio do valor médio entre clubes, mais financeiramente paritária.",
    sortKey: "score",
    sortDir: "desc",
    columns: [
      {
        key: "score",
        label: "Score Paridade",
        value: (r, x) => x.score?.[r.league] ?? 0,
        fmt: "dec1",
      },
    ...
    extras: (rows) => ({
      score: toMap(rows, (r) => 100 - (normalize(rows, (x) => x.valueStd).get(r.league) ?? 0)),
    }),
  },
  {
    key: "parityTech",
    label: "Paridade Técnica",
    description: "Quanto menor o desvio do CA médio entre clubes, mais paritária tecnicamente.",
    sortKey: "score",
    sortDir: "desc",
    columns: [
      {
        key: "score",
        label: "Score Paridade Téc.",
        value: (r, x) => x.score?.[r.league] ?? 0,
        fmt: "dec1",
      },
    ...
    extras: (rows) => ({
      score: toMap(rows, (r) => 100 - (normalize(rows, (x) => x.caStd).get(r.league) ?? 0)),
    }),
  },
  {
    key: "dominance",
    label: "Dominância",
    description:
      "Diferença em pontos do líder para o 2º na última época — quanto maior, mais dominante.",
    sortKey: "leaderGap",
    sortDir: "desc",
    columns: [
  ...
  {
    key: "offEff",
    label: "Eficiência Ofensiva",
    description:
      "Pontos conquistados por golo marcado — eficiência ao converter ataque em resultados.",
    sortKey: "offEfficiency",
    sortDir: "desc",
    columns: [
  ...
  },
  {
    ...
    description: "Menos golos sofridos por jogo — ligas defensivamente sólidas.",
    sortKey: "gsPerGame",
    sortDir: "asc",
    columns: [
      { key: "gsTotal", label: "GS", value: (r) => r.gsTotal, fmt: "int" },
