...
function standingWeighted(
  ...
  if (s.is_champion) {
    const bonus =
      s.module === "superleague" ? cfg.superleagueChampionBonus : cfg.nationalChampionBonus;
    weighted += bonus * compW * divW * decay;
  }
  if (
    s.module === "superleague" &&
    cfg.superleaguePromotionBonus > 0 &&
    hasPromotionToken(s.info)
  ) {
...
export function buildYearMaps(
  data: AllData,
  cfg: FmConfig = DEFAULT_CONFIG,
  ...
  if (source === "international") {
    ...
    for (const r of data.international ?? []) {
      const { weight } = (function () {
        ...
        const w =
          cfg.internationalWeights.find((t) => new RegExp(t.match, "i").test(r.competition))
            ?.weight ?? 100;
        return { weight: w };
      })();
...
export function buildClubProfile(
  data: AllData,
  name: string,
  cfg: FmConfig = DEFAULT_CONFIG,
): ClubProfile | null {
  ...
  for (const c of data.continental) {
    ...
    if (sfs.includes(name))
      knockouts.push({ year: c.season_year, competition: c.competition, stage: "SF", club: name });
    if (qfs.includes(name))
      knockouts.push({ year: c.season_year, competition: c.competition, stage: "QF", club: name });
  }
  ...
  for (const s of seasons) {
    ...
    if (s.position != null && (bestPosition == null || s.position < bestPosition))
...
export function buildCoachProfile(
  ...
  for (const r of data.international) {
    if (r.sf1_coach === name && r.sf1)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "SF", club: r.sf1 });
    if (r.sf2_coach === name && r.sf2)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "SF", club: r.sf2 });
    if (r.qf1_coach === name && r.qf1)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "QF", club: r.qf1 });
    if (r.qf2_coach === name && r.qf2)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "QF", club: r.qf2 });
    if (r.qf3_coach === name && r.qf3)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "QF", club: r.qf3 });
    if (r.qf4_coach === name && r.qf4)
      knockouts.push({ year: r.season_year, competition: r.competition, stage: "QF", club: r.qf4 });
  }
  knockouts.sort((a, b) => b.year - a.year || a.competition.localeCompare(b.competition));
  ...
  for (const t of continentalTitles) {
    ...
    if (
      t.role === "winner" &&
      !seasons.some(
        (s) => s.year === t.year && s.club_name === t.club && s.module === "continental",
      )
    ) {
      titles++;
    }
  ...
  const clubs = [
    ...new Set([
      ...(assigns.map((a) => a.club_name).filter(Boolean) as string[]),
      ...(intlRows.map((r) => (r.coach1 === name ? r.team1 : r.team2)).filter(Boolean) as string[]),
    ]),
  ];
  const byYearW = new Map<number, number>();
  const byYearR = new Map<number, number>();
