...
export function normalizeDesafio(d: any): Desafio {
  const subjects: Subject[] =
    Array.isArray(d.subjects) && d.subjects.length > 0
      ...
      : d.subject && ALL_SUBJECTS.includes(d.subject)
  ...
  return {
    ...
    requirements: (d.requirements ?? []).map((r: any) => ({
      type: (Object.keys(REQ_LABEL).includes(r?.type)
        ? r.type
        : r?.type === "superleague"
          ? "superleague-champion"
          : r?.type === "national"
      ...
      leagueScope: (r?.leagueScope === "superleague" || r?.leagueScope === "national"
        ? r.leagueScope
        : "any") as LeagueScope,
    })),
  };
...
export function buildDesafioIndex(data: any, ranks: ComputeResult) {
  ...
  for (const s of data.standings) {
    if (s.module === "superleague") {
      if (s.is_champion) {
        ...
        set.add(s.season_year);
        clubSLChampion.set(s.club_name, set);
      }
      const tokens = s.info
        ? new Set(
            String(s.info)
      ...
      if (s.is_champion || tokens.has("P")) {
        ...
        set.add(s.season_year);
        clubSLPromOrChamp.set(s.club_name, set);
      }
    } else if (s.module === "national" && s.is_champion) {
      clubNationalChampion.push({
        league: s.division_label ?? "",
        club: s.club_name,
        year: s.season_year,
      });
    }
  ...
  for (const c of data.continental) {
    if (c.winner)
      clubContinentalWin.push({ comp: c.competition, club: c.winner, year: c.season_year });
  }

  const intlWinByCountry: { comp: string; country: string; year: number }[] = [];
  for (const r of data.international ?? []) {
    if (r.winner)
      intlWinByCountry.push({ comp: r.competition, country: r.winner, year: r.season_year });
  }
  ...
  const hofGet = (cat: Subject, n: number) => {
    const cache =
      cat === "clubs" ? hofClubsCache : cat === "coaches" ? hofCoachesCache : hofCountriesCache;
    const evo =
      cat === "clubs"
...
function evaluateForSubject(
  ...
  for (const entity of candidates) {
    ...
    for (const req of desafio.requirements) {
      ...
      if (req.type === "hall-of-fame") {
        ...
        const matchClub = (club: string, year: number) => {
          ...
          if (subject === "coaches") {
            return (
              idx.coachByClubSeason.get(`superleague|${year}|${club}`) === entity ||
        ...
        for (const r of pool) {
          ...
          extras.push(
            `${r.year} · ${r.league}${who} · ${r.points} pts · ${r.losses}D / ${r.played}J · ${prefix}`,
          );
        }
        const scopeLbl = scope === "any" ? "" : scope === "superleague" ? " SL" : " Nac.";
        perReqDetails.push(
          `${req.type === "unbeaten-season" ? "Invencível" : "Recorde pts"}${scopeLbl}${matchTxt ? ` (${req.match})` : ""}`,
        );
        ...
        if (req.type === "superleague-champion") {
          for (const [club, ys] of idx.clubSLChampion)
            for (const y of ys) if (coachWonAt("superleague", club, y)) years.add(y);
          perReqDetails.push("SL campeão (qualquer clube)");
        } else if (req.type === "superleague-promotion") {
          for (const [club, ys] of idx.clubSLPromOrChamp)
            for (const y of ys) if (coachWonAt("superleague", club, y)) years.add(y);
          ...
          for (const r of idx.clubContinentalWin) {
            if (matchTxt && !r.comp.toLowerCase().includes(matchTxt)) continue;
            if (idx.coachByClubSeason.get(`superleague|${r.year}|${r.club}`) === coach)
              years.add(r.year);
          }
        ...
        if (req.type === "international-winner") {
          ...
          for (const [club, ys] of idx.clubSLChampion)
            if (idx.clubCountry[club] === country) for (const y of ys) years.add(y);
    ...
    if (!ok || perReqYears.length !== desafio.requirements.length || perReqYears.length === 0)
      continue;
...
export function evaluateDesafios(
  data: any,
  ranks: ComputeResult,
  list: Desafio[],
): DesafioResult[] {
...
export function applyDesafioBonuses(ranks: ComputeResult, results: DesafioResult[]): ComputeResult {
  ...
  for (const { desafio, matches } of results) {
    ...
    for (const m of matches) {
      const arr =
        m.subject === "clubs" ? out.clubs : m.subject === "coaches" ? out.coaches : out.countries;
...
export function buildDesafioBreakdownBySubject(
  ...
  const out: Record<
    string,
    { total: number; items: { name: string; bonus: number; years: number[] }[] }
  > = {};
  for (const { desafio, matches } of results) {
    if (!desafio.bonus) continue;
    for (const m of matches) {
...
export const DESAFIO_PRESETS: Desafio[] = [
  {
    id: "preset-tri-campeao-superleague",
    name: "Tri-campeão da SuperLeague",
    description: "Vencer a SuperLeague em 3 épocas consecutivas (clube ou treinador).",
    subjects: ["clubs", "coaches"],
    sameYear: false,
    bonus: 1500,
    requirements: [{ type: "superleague-champion", match: "", count: 3, consecutive: true }],
  },
  {
    id: "preset-penta-campeao-superleague",
    name: "Penta-campeão da SuperLeague",
  ...
  {
    id: "preset-dinastia-continental",
    name: "Dinastia Continental",
  ...
  {
    id: "preset-dobradinha-continental",
    name: "Dobradinha Continental",
  ...
  {
    id: "preset-lenda-hall-of-fame",
    name: "Lenda do Hall of Fame",
  ...
  {
    id: "preset-elite-hall-of-fame",
    name: "Elite do Hall of Fame",
  ...
  {
    id: "preset-invencivel-superleague",
    name: "Invencível na SuperLeague",
    ...
    requirements: [
      {
        type: "unbeaten-season",
        match: "",
        count: 1,
        consecutive: false,
        leagueScope: "superleague",
      },
    ],
  },
  {
    id: "preset-invencivel-nacional",
    name: "Invencível Nacional",
    description: "Vencer uma liga nacional sem perder.",
    subjects: ["clubs", "coaches"],
    sameYear: false,
    bonus: 800,
    requirements: [
      { type: "unbeaten-season", match: "", count: 1, consecutive: false, leagueScope: "national" },
    ],
  },
  {
    id: "preset-recorde-pontos",
    name: "Recorde de pontos",
    description: "Bater o recorde de pontos numa liga/divisão.",
    subjects: ["clubs", "coaches"],
    sameYear: false,
    bonus: 600,
    requirements: [
      { type: "points-record", match: "", count: 1, consecutive: false, leagueScope: "any" },
    ],
  },
  {
    id: "preset-promocao-express",
    name: "Promoção Express",
  ...
  {
    id: "preset-gloria-internacional",
    name: "Glória Internacional",
  ...
  {
    id: "preset-bicampeao-internacional",
    name: "Bicampeão Internacional",
  ...
  {
    id: "preset-imperio-de-um-pais",
    name: "Império de um país",
    description:
      "Selecção + Clube do país vencem competições internacional e continental no mesmo ano.",
    subjects: ["countries"],
    sameYear: true,
    bonus: 3000,
    requirements: [
  ...
  {
    id: "preset-colecao-continental",
    name: "Coleção Continental",
