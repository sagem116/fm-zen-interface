  ...
  const extras = useQuery({
    ...
    queryFn: async () => {
      const [clubRep, compRep] = await Promise.all([
        fetchAllRows<ClubReputationRow>(
          "club_reputation_season",
          "club_name,season_year,reputation,avg_attendance,season_ticket_holders",
        ),
        fetchAllRows<CompetitionReputationRow>(
          "competition_reputation",
          "competition,season_year,reputation,country,continent",
        ),
      ]);
  ...
  const data: AnalyticsData | null =
    ranks.data && ps.data && extras.data
      ? {
          ...
          rankClubs: ranks.data.ranks.clubs.map((e) => ({
            name: e.name,
            weighted: e.weighted,
            titles: e.titles,
          })),
          rankCoaches: ranks.data.ranks.coaches.map((e) => ({
            name: e.name,
            weighted: e.weighted,
            titles: e.titles,
          })),
          rankCountries: ranks.data.ranks.countries.map((e) => ({
            name: e.name,
            weighted: e.weighted,
            titles: e.titles,
          })),
...
function stdIn(
  data: AnalyticsData,
  seasons: Set<number>,
  modules: StandingRow["module"][] = ["superleague", "national"],
) {
...
export function clubRankings(
  ...
  for (const [club, rows] of playersByClub) {
    ...
    for (const p of rows) {
      ...
      a.xg += N(p.xg);
      if (p.pass_pct != null) {
        a.passSum += N(p.pass_pct);
        a.passN++;
      }
      a.tackles += N(p.tackles_per90);
      if (p.avg_rating != null) {
        a.ratingSum += N(p.avg_rating);
        a.ratingN++;
      }
      a.yellows += N(p.yellows);
      a.reds += N(p.reds);
  ...
  const ataque: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(clubs, (c) => c.gf),
        (c) => ({ name: cl(c.club), value: c.gf, subtitle: `${c.played} jogos` }),
      ),
    },
    {
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          [...clubPlayers.entries()].map(([club, a]) => ({ club, xg: a.xg })),
          (c) => c.xg,
        ),
    ...
    },
    {
      ...
      rows: toRows(
        ...
        (r) => ({
          name: cl(r.club),
          value: r.ratio,
          subtitle: `${r.gls.toFixed(0)}g / ${r.xg.toFixed(1)}xG`,
        }),
      ),
    },
    {
      ...
      rows: (() => {
        ...
        for (const p of players) {
          ...
          cur.sum += N(p.shot_pct);
          cur.n++;
          acc.set(p.club, cur);
        }
        const list = [...acc.values()].filter((r) => r.n > 0);
        return toRows(
          topN(list, (r) => r.sum / r.n),
          (r) => ({ name: r.club, value: r.sum / r.n }),
        );
      })(),
    },
    {
      ...
      rows: toRows(
        topN(
          [...clubPlayers.entries()].map(([club, a]) => ({ club, ast: a.ast })),
          (c) => c.ast,
        ),
  ...
  const defesa: Ranking[] = [
    {
      ...
      rows: toRows(
        ...
        (c) => ({ name: cl(c.club), value: c.ga, subtitle: `${c.played} jogos` }),
      ),
    },
    {
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          clubs.filter((c) => c.ga > 0),
          (c) => c.points / c.ga,
        ),
        (c) => ({
          name: cl(c.club),
          value: c.points / c.ga,
          subtitle: `${c.points}pt / ${c.ga}gs`,
        }),
      ),
    },
    {
      ...
      rows: toRows(
        ...
        (c) => ({ name: cl(c.club), value: c.losses, subtitle: `${c.played} jogos` }),
      ),
    },
    {
      id: "clubs-gd",
      ...
      linkKind: "club",
      rows: toRows(
        topN(clubs, (c) => c.gf - c.ga),
        (c) => ({ name: cl(c.club), value: c.gf - c.ga }),
      ),
    },
  ];
  ...
  const eficiencia: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(
          clubs.filter((c) => c.gf > 0),
          (c) => c.points / c.gf,
        ),
        (c) => ({ name: cl(c.club), value: c.points / c.gf }),
      ),
    },
    {
      ...
      rows: toRows(
        topN(
          clubs.filter((c) => c.wins > 0),
          (c) => c.gf / c.wins,
          10,
          "asc",
        ),
        (c) => ({ name: cl(c.club), value: c.gf / c.wins }),
      ),
    },
    {
      id: "clubs-points-max",
  ...
  const economia: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(
          [...clubPlayers.entries()].map(([club, a]) => ({ club, vp: a.vpSum })),
          (r) => r.vp,
        ),
  ...
  const plantel: Ranking[] = [
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          [...clubPlayers.entries()].map(([club, a]) => ({ club, n: a.nats.size })),
          (r) => r.n,
        ),
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          [...clubPlayers.entries()].map(([club, a]) => ({ club, t: a.tackles })),
          (r) => r.t,
        ),
...
export function competitionRankings(data: AnalyticsData, seasonSet: Set<number>) {
  ...
  for (const c of cs) {
    const cur = map.get(c.competition) ?? {
      comp: c.competition,
      nSeasons: 0,
      xg: 0,
      pass: 0,
  ...
  const compValue = new Map<
    string,
    { clubsVpSum: number; clubsSet: Set<string>; playerVpAvg: number; playerN: number }
  >();
  for (const [comp, ps] of playersByComp) {
    ...
    compValue.set(comp, {
      clubsVpSum: vpSum,
      clubsSet: clubs,
      playerVpAvg: playerN > 0 ? playerVpSum / playerN : 0,
      playerN,
    });
  }
  ...
  const qualidade: Ranking[] = [
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          [...compValue.entries()].map(([comp, a]) => ({ comp, v: a.clubsVpSum })),
          (r) => r.v,
        ),
  ...
  const competitividade: Ranking[] = [
      ...
      rows: toRows(
        topN(
          [...gapMap.entries()]
            .map(([comp, a]) => ({
              comp,
              gap: a.gaps.length > 0 ? a.gaps.reduce((s, x) => s + x, 0) / a.gaps.length : 0,
            }))
            .filter((r) => r.gap > 0),
          (r) => r.gap,
          10,
    ...
    },
    {
      ...
      rows: toRows(
        topN(
          [...champions.entries()].map(([comp, s]) => ({ comp, n: s.size })),
          (r) => r.n,
        ),
  ...
  const estilo: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(
          comps.filter((c) => c.xgAvg > 0),
          (c) => c.xgAvg,
        ),
        (c) => ({ name: c.comp, value: c.xgAvg }),
      ),
    },
    {
      id: "comp-pass",
      ...
      rows: toRows(
        topN(
          comps.filter((c) => c.passAvg > 0),
          (c) => c.passAvg,
        ),
        (c) => ({ name: c.comp, value: c.passAvg }),
      ),
    },
    {
      id: "comp-tackles",
      ...
      rows: toRows(
        topN(
          comps.filter((c) => c.tacklesAvg > 0),
          (c) => c.tacklesAvg,
        ),
        (c) => ({ name: c.comp, value: c.tacklesAvg }),
      ),
    },
    {
      id: "comp-rating2",
      ...
      rows: toRows(
        topN(
          comps.filter((c) => c.ratingAvg > 0),
          (c) => c.ratingAvg,
        ),
        (c) => ({ name: c.comp, value: c.ratingAvg }),
      ),
    },
    {
      id: "comp-cards",
      ...
      rows: toRows(
        topN(
          comps
            .map((c) => ({ ...c, cards: c.yellowsAvg + 3 * c.redsAvg }))
            .filter((c) => c.cards > 0),
          (c) => c.cards,
        ),
        (c) => ({ name: c.comp, value: c.cards }),
      ),
    },
    {
  ...
  const economiaC: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(
          comps.filter((c) => c.salaryAvg > 0),
          (c) => c.salaryAvg,
        ),
        (c) => ({ name: c.comp, value: c.salaryAvg }),
      ),
    },
    {
      id: "comp-vp-total",
...
export function playerRankings(data: AnalyticsData, seasonSet: Set<number>) {
  ...
  for (const p of players) {
    const cur = map.get(p.player_name) ?? {
      name: p.player_name,
      club: p.club,
      gls: 0,
      ast: 0,
    ...
    cur.hdj += N(p.hdj);
    if (p.avg_rating != null) {
      cur.ratingSum += N(p.avg_rating);
      cur.ratingN++;
    }
    cur.games += N(p.games);
    if (p.pass_pct != null) {
      cur.passSum += N(p.pass_pct);
      cur.passN++;
    }
    cur.tackles += N(p.tackles_per90);
    if (p.shot_pct != null) {
      cur.shot += N(p.shot_pct);
      cur.shotN++;
    }
    cur.vpMax = Math.max(cur.vpMax, N(p.vp));
    cur.salaryMax = Math.max(cur.salaryMax, N(p.salary));
    cur.comps.add(p.competition);
    cur.seasons.add(p.season_year);
  ...
  const ofensivos: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(arr, (p) => p.gls),
        (p) => ({ name: p.name, value: p.gls, subtitle: p.club ?? undefined }),
      ),
    },
    {
      id: "p-ast",
      ...
      rows: toRows(
        topN(arr, (p) => p.ast),
        (p) => ({ name: p.name, value: p.ast, subtitle: p.club ?? undefined }),
      ),
    },
    {
      id: "p-gxg",
      title: "Melhor relação Golos/xG",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.xg > 0.5),
          (p) => p.gls / p.xg,
        ),
        (p) => ({
          name: p.name,
          value: p.gls / p.xg,
          subtitle: `${p.gls}g / ${p.xg.toFixed(1)}xG`,
        }),
      ),
    },
    {
      id: "p-goal-contrib",
      title: "Maior contribuição ofensiva (G+A)",
      fmt: "int",
      linkKind: "player",
      rows: toRows(
        topN(arr, (p) => p.gls + p.ast),
        (p) => ({ name: p.name, value: p.gls + p.ast }),
      ),
    },
    {
      id: "p-hdj",
      title: "Mais Homem do Jogo",
      fmt: "int",
      linkKind: "player",
      rows: toRows(
        topN(arr, (p) => p.hdj),
        (p) => ({ name: p.name, value: p.hdj }),
      ),
    },
    {
      id: "p-rating",
      title: "Melhor rating médio",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.ratingN > 0),
          (p) => p.ratingSum / p.ratingN,
        ),
  ...
  const tecnicos: Ranking[] = [
    {
      id: "p-pass",
      title: "Melhor % de passe",
      fmt: "num1",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.passN > 0 && p.games >= 5),
          (p) => p.passSum / p.passN,
        ),
    ...
    {
      id: "p-shots",
      title: "Melhor % de remates convertidos",
      fmt: "pct",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.shotN > 0 && p.gls > 2),
          (p) => p.shot / p.shotN,
        ),
    ...
    {
      id: "p-tackles",
      title: "Melhor desempenho defensivo (Des/90)",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.games >= 5),
          (p) => p.tackles / Math.max(1, p.seasons.size),
        ),
  ...
  const mercado: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(arr, (p) => p.vpMax),
        (p) => ({ name: p.name, value: p.vpMax, subtitle: p.club ?? undefined }),
      ),
    },
    {
      id: "p-vp-per-perf",
      title: "Melhor relação Desempenho / VP (pontos ofensivos por M€)",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr
            .filter((p) => p.vpMax > 0)
            .map((p) => ({ ...p, eff: (p.gls + 0.5 * p.ast) / (p.vpMax / 1_000_000) })),
          (p) => p.eff,
        ),
    ...
    {
      id: "p-sal-per-perf",
      title: "Melhor relação Desempenho / Salário",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr
            .filter((p) => p.salaryMax > 0)
            .map((p) => ({ ...p, eff: (p.gls + 0.5 * p.ast) / (p.salaryMax / 1_000_000) })),
          (p) => p.eff,
        ),
        (p) => ({ name: p.name, value: p.eff }),
      ),
  ...
  const carreira: Ranking[] = [
    {
      id: "p-longevity",
      title: "Maior longevidade (épocas)",
      fmt: "int",
      linkKind: "player",
      rows: toRows(
        topN(arr, (p) => p.seasons.size),
        (p) => ({ name: p.name, value: p.seasons.size }),
      ),
    },
    {
      id: "p-comps",
      title: "Mais competições disputadas",
      fmt: "int",
      linkKind: "player",
      rows: toRows(
        topN(arr, (p) => p.comps.size),
        (p) => ({ name: p.name, value: p.comps.size }),
      ),
    },
    {
      id: "p-consistent",
      title: "Mais consistentes (rating × épocas)",
      fmt: "num2",
      linkKind: "player",
      rows: toRows(
        topN(
          arr.filter((p) => p.ratingN > 0 && p.seasons.size >= 1),
          (p) => (p.ratingSum / p.ratingN) * p.seasons.size,
        ),
...
export function coachRankings(data: AnalyticsData, seasonSet: Set<number>) {
  ...
  for (const c of coaches) {
    ...
    if (c.club_name) {
      ...
      if (s) {
        cur.played += N(s.played);
        cur.wins += N(s.wins);
        cur.draws += N(s.draws);
        cur.losses += N(s.losses);
        cur.gf += N(s.gf);
        cur.ga += N(s.ga);
        cur.points += N(s.points);
  ...
  const rankings: Ranking[] = [
    {
      ...
      rows: toRows(
        topN(arr, (c) => c.titles),
        (c) => ({ name: c.name, value: c.titles }),
      ),
    },
    {
      id: "co-win-pct",
      title: "Maior % de vitórias",
      fmt: "pct",
      linkKind: "coach",
      rows: toRows(
        topN(
          arr.filter((c) => c.played >= 10),
          (c) => (c.wins / c.played) * 100,
        ),
        (c) => ({
          name: c.name,
          value: (c.wins / c.played) * 100,
          subtitle: `${c.wins}v em ${c.played}j`,
        }),
      ),
    },
    {
    ...
    {
      id: "co-gf-pg",
      title: "Mais golos marcados por jogo",
      fmt: "num2",
      linkKind: "coach",
      rows: toRows(
        topN(
    ...
    {
      id: "co-ga-pg",
      title: "Menos golos sofridos por jogo",
      fmt: "num2",
      linkKind: "coach",
      higherIsBetter: false,
      rows: toRows(
    ...
    },
    {
      ...
      linkKind: "coach",
      rows: toRows(
        topN(arr, (c) => c.seasons.size),
        (c) => ({ name: c.name, value: c.seasons.size }),
      ),
    },
    {
      id: "co-clubs",
      title: "Mais clubes treinados",
      fmt: "int",
      linkKind: "coach",
      rows: toRows(
        topN(arr, (c) => c.clubs.size),
        (c) => ({ name: c.name, value: c.clubs.size }),
      ),
    },
    {
      id: "co-nats",
      title: "Mais seleções treinadas",
      fmt: "int",
      linkKind: "coach",
      rows: toRows(
        topN(arr, (c) => c.countries.size),
        (c) => ({ name: c.name, value: c.countries.size }),
      ),
    },
    {
      id: "co-consistent",
      title: "Treinadores mais consistentes (Pontos × Épocas)",
      fmt: "num2",
      linkKind: "coach",
      rows: toRows(
        topN(
          arr.filter((c) => c.played > 0),
          (c) => (c.points / c.played) * c.seasons.size,
        ),
...
export function countryRankings(data: AnalyticsData, seasonSet: Set<number>) {
  ...
  const rankings: Ranking[] = [
    {
      id: "ctry-rank",
      title: "Melhores países (ranking global)",
      fmt: "num2",
      linkKind: "country",
      rows: toRows(
        topN(bestCountriesByRank, (c) => c.weighted),
        (c) => ({ name: c.name, value: c.weighted }),
      ),
    },
    {
      id: "ctry-clubs",
      title: "Países com mais clubes ativos",
      fmt: "int",
      linkKind: "country",
      rows: toRows(
        topN(arr, (a) => a.clubs.size),
        (a) => ({ name: a.country, value: a.clubs.size }),
      ),
    },
    {
      id: "ctry-comps",
      title: "Países com mais competições",
      fmt: "int",
      linkKind: "country",
      rows: toRows(
        topN(arr, (a) => a.competitions.size),
        (a) => ({ name: a.country, value: a.competitions.size }),
      ),
    },
    {
      id: "ctry-vp",
      title: "Países com maior valor de mercado (clubes)",
      fmt: "money",
      linkKind: "country",
      rows: toRows(
        topN(arr, (a) => a.vpSum),
        (a) => ({ name: a.country, value: a.vpSum }),
      ),
    },
    {
      id: "ctry-players-nat",
      title: "Países que exportam mais jogadores",
      fmt: "int",
      linkKind: "country",
      rows: toRows(
        topN(arr, (a) => a.players.size),
        (a) => ({ name: a.country, value: a.players.size }),
      ),
    },
    {
      id: "ctry-titles",
      title: "Países com mais títulos internacionais",
      fmt: "int",
      linkKind: "country",
      rows: toRows(
        topN(bestCountriesByRank, (c) => c.titles),
        (c) => ({ name: c.name, value: c.titles }),
      ),
    },
  ];
...
export function evolutionSeries(data: AnalyticsData): EvoSeries[] {
  ...
  const perSeason = (year: number) => {
    const std = data.standings.filter(
      (s) => s.season_year === year && (s.module === "superleague" || s.module === "national"),
    );
    ...
    for (const p of players) {
      ...
      xg += N(p.xg);
      if (p.avg_rating != null) {
        ratingSum += N(p.avg_rating);
        ratingN++;
      }
      salarySum += N(p.salary);
      vpSum += N(p.vp);
      if (p.age != null) {
        ageSum += N(p.age);
        ageN++;
    ...
    return {
      ...
      salarySum,
      vpSum,
      ageAvg: ageN > 0 ? ageSum / ageN : 0,
      caAvg: caN > 0 ? caSum / caN : 0,
      playersN: players.length,
      clubsN: clubs.size,
      coachesN: coachCount,
      ratingAvg: ratingN > 0 ? ratingSum / ratingN : 0,
      repAvg,
    };
  };
...
export function computeRecords(data: AnalyticsData): RecordRow[] {
  ...
  if (bestPts)
    records.push({
      label: "Clube com mais pontos numa época",
      entity: bestPts.club,
      value: bestPts.points,
      fmt: "int",
      context: `${bestPts.year}`,
    });
  if (mostGf)
    records.push({
      label: "Clube com melhor ataque numa época",
      entity: mostGf.club,
      value: mostGf.gf,
      fmt: "int",
      context: `${mostGf.year}`,
  ...
  if (lessGa)
    records.push({
      label: "Clube com melhor defesa numa época (menos golos sofridos)",
      entity: lessGa.club,
      value: lessGa.ga,
      fmt: "int",
      context: `${lessGa.year}`,
  ...
  if (bestGd)
    records.push({
      label: "Melhor diferença de golos numa época",
      entity: bestGd.club,
      value: bestGd.gd,
      fmt: "int",
      context: `${bestGd.year}`,
  ...
  if (mostWins)
    records.push({
      label: "Mais vitórias numa época",
      entity: mostWins.club,
      value: mostWins.wins,
      fmt: "int",
      context: `${mostWins.year}`,
    });
  if (mostGls)
    records.push({
      label: "Jogador com mais golos numa época",
      entity: mostGls.name,
      value: mostGls.gls,
      fmt: "int",
      context: `${mostGls.year}${mostGls.club ? ` (${mostGls.club})` : ""}`,
    });
  if (mostAst)
    records.push({
      label: "Jogador com mais assistências numa época",
      entity: mostAst.name,
      value: mostAst.ast,
      fmt: "int",
      context: `${mostAst.year}${mostAst.club ? ` (${mostAst.club})` : ""}`,
    });
  if (mostVp)
    records.push({
      label: "Jogador mais valioso alguma vez registado",
      entity: mostVp.name,
      value: mostVp.vp,
      fmt: "money",
      context: `${mostVp.year}`,
  ...
  if (topCoach)
    records.push({
      label: "Treinador com mais pontos ponderados",
      entity: topCoach.name,
      value: topCoach.weighted,
      fmt: "num2",
    });
  const topTitles = [...data.rankCoaches].sort((a, b) => b.titles - a.titles)[0];
  if (topTitles)
    records.push({
      label: "Treinador com mais títulos",
      entity: topTitles.name,
      value: topTitles.titles,
      fmt: "int",
    });
...
export function computeCuriosities(data: AnalyticsData): Curiosity[] {
  ...
  if (vpDelta.length) {
    ...
    items.push({
      label: "Clube que mais evoluiu (valor)",
      text: `${up.club} valorizou-se ${(up.delta / 1_000_000).toFixed(1)} M€ de ${prev} para ${last}.`,
    });
    if (down.delta < 0) {
      items.push({
        label: "Clube que mais perdeu valor",
        text: `${down.club} perdeu ${(Math.abs(down.delta) / 1_000_000).toFixed(1)} M€ de ${prev} para ${last}.`,
      });
    }
  }
  ...
  const ppPrev = playerVp(prev),
    ppLast = playerVp(last);
  ...
  if (ppDelta.length) {
    const up = ppDelta[0];
    items.push({
      label: "Jogador que mais valorizou",
      text: `${up.name} valorizou-se ${(up.delta / 1_000_000).toFixed(1)} M€ de ${prev} para ${last}.`,
    });
  }
  ...
  if (nDelta.length && nDelta[0].delta > 0) {
    items.push({
      label: "País em maior crescimento",
      text: `${nDelta[0].name} aumentou em ${nDelta[0].delta} o número de jogadores no ativo.`,
    });
  }
  ...
  if (topClub)
    items.push({
      label: "Clube mais dominante da história",
      text: `${topClub.name} lidera o ranking mundial com ${topClub.weighted.toFixed(1)} pontos ponderados.`,
    });
  const topCoach = data.rankCoaches[0];
  if (topCoach)
    items.push({
      label: "Treinador mais vencedor",
      text: `${topCoach.name} soma ${topCoach.titles} título(s) e ${topCoach.weighted.toFixed(1)} pontos ponderados.`,
    });
  ...
  const posPrev = posByClubComp(prev),
    posLast = posByClubComp(last);
  ...
  if (posDelta.length) {
    const s = posDelta[0];
    const [club, comp] = s.key.split("|");
    if (s.delta > 0)
      items.push({
        label: "Maior surpresa da época",
        text: `${club} subiu ${s.delta} posições em ${comp || "campeonato"} de ${prev} para ${last}.`,
      });
    const w = posDelta[posDelta.length - 1];
    if (w.delta < 0) {
      const [wclub, wcomp] = w.key.split("|");
      items.push({
        label: "Maior queda da época",
        text: `${wclub} caiu ${Math.abs(w.delta)} posições em ${wcomp || "campeonato"} de ${prev} para ${last}.`,
      });
    }
  }
