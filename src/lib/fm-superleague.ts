...
export function computeClubPlayoffs(standings: StandingRow[]): PlayoffRow[] {
  ...
  for (const s of slStandings(standings)) {
    ...
    if (div > 1 && pos >= 2 && pos <= 5 && !isP(s.info)) {
      r.quaseSubida++;
      r._QS.push(`${s.season_year} (Div ${div}, ${pos}º)`);
    }
    if (div === 1 && pos >= 1 && pos <= 2 && !isC(s.info)) {
      r.quaseTitulo++;
      r._QT.push(`${s.season_year} (${pos}º)`);
    }
    map.set(s.club_name, r);
  }
  return [...map.values()]
...
export function computeCoachChampions(standings: StandingRow[], coaches: CoachRow[]): ChampRow[] {
  ...
  for (const s of slStandings(standings)) {
    ...
    if (isC(s.info)) {
      r.c++;
      r._C.push(`${s.club_name} — ${s.season_year} (Div ${s.division_num ?? "?"}${posTag})`);
    }
    if (isP(s.info)) {
      r.p++;
      r._P.push(`${s.club_name} — ${s.season_year} (Div ${s.division_num ?? "?"}${posTag})`);
    }
    if (isD(s.info)) {
      r.d++;
      r._D.push(`${s.club_name} — ${s.season_year} (Div ${s.division_num ?? "?"}${posTag})`);
    }
    map.set(tr.name, r);
  }
  return [...map.values()]
...
export function computeCoachPlayoffs(standings: StandingRow[], coaches: CoachRow[]): PlayoffRow[] {
  ...
  for (const s of slStandings(standings)) {
    ...
    if (div > 1 && pos >= 2 && pos <= 5 && !isP(s.info)) {
      r.quaseSubida++;
      r._QS.push(`${s.club_name} — ${s.season_year} (Div ${div}, ${pos}º)`);
    }
    if (div === 1 && pos >= 1 && pos <= 2 && !isC(s.info)) {
      r.quaseTitulo++;
      r._QT.push(`${s.club_name} — ${s.season_year} (${pos}º)`);
    }
    map.set(tr.name, r);
  }
  return [...map.values()]
