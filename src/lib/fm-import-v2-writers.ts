...
import type { ParsedCompetitionsFile, ParsedPlayersFile, ClubCountryRow } from "./fm-import-v2";
import { hydrateClubReputationsFromDb } from "./fm-club-reputation";
import { hydrateCompetitionReputationsFromDb } from "./fm-competition-reputation";
...
export interface CompetitionsImportOptions {
  scope?: "all" | "clubs" | "coaches" | "competitions" | "classifications";
  filename?: string;
}
...
export async function importCompetitionsFile(
  ...
  options: CompetitionsImportOptions = {},
): Promise<CompetitionsImportResult> {
  const scope = options.scope ?? "all";
  const shouldImportClubs = scope === "all" || scope === "clubs";
  const shouldImportCoaches = scope === "all" || scope === "coaches";
  const shouldImportCompetitions = scope === "all" || scope === "competitions";
  const shouldImportClassifications = scope === "all" || scope === "classifications";
  ...
  parsed.coaches.forEach((c) => {
    if (c.club) allClubs.add(c.club);
  });
  ...
  [...parsed.continental, ...parsed.international].forEach((b) => {
    ...
    [b.sf1, b.sf2, b.qf1, b.qf2, b.qf3, b.qf4].forEach((n) => {
      if (n) allClubs.add(n);
    });
  });
  ...
  if (missing.length) {
    ...
    ((extra ?? []) as Array<{ id: string; name: string }>).forEach((c) =>
      clubMap.set(c.name, c.id),
    );
  }

  // ---- Delete previous slices for this season ----
  const del: Array<Promise<{ error: { message: string } | null }>> = [];
  if (shouldImportClassifications) {
    del.push(
      sb()
        .from("standings")
        .delete()
    ...
    del.push(sb().from("continental_results").delete().eq("season_id", seasonId));
    del.push(sb().from("international_results").delete().eq("season_id", seasonId));
  }
  if (shouldImportClubs) {
    del.push(sb().from("club_reputation_season").delete().eq("season_id", seasonId));
  }
  if (shouldImportCoaches) {
    del.push(sb().from("coach_assignments").delete().eq("season_id", seasonId));
  }
  if (shouldImportCompetitions) {
    del.push(sb().from("competition_reputation").delete().eq("season_year", year));
  }
  del.push(sb().from("imports").delete().eq("season_id", seasonId).eq("module", scope));
  for (const p of del) {
    const { error } = await p;
    if (error) throw new Error(`delete previous: ${error.message}`);
  }
  ...
  if (shouldImportClassifications && standingsPayload.length)
  ...
  if (shouldImportClassifications && contPayload.length)
  ...
  if (shouldImportClassifications && intPayload.length)
  ...
  for (const c of parsed.coaches) {
    ...
    if (!uniqueCoaches.has(key))
      uniqueCoaches.set(key, {
        name: c.name,
  ...
  if (shouldImportCoaches && uniqueCoaches.size) {
    const { error } = await sb()
      .from("coaches")
      .upsert([...uniqueCoaches.values()], { onConflict: "name,nationality" });
    if (error) throw new Error(`coaches: ${error.message}`);
  }
  const { data: coachRows } = shouldImportCoaches
    ? await sb().from("coaches").select("id,name,nationality")
    : { data: [] as Array<{ id: string; name: string; nationality: string | null }> };
  const coachMap = new Map(
    ((coachRows ?? []) as Array<{ id: string; name: string; nationality: string | null }>).map(
      (c) => [`${c.name}|${c.nationality ?? ""}`, c.id],
    ),
  );
  ...
  if (shouldImportCoaches && assignmentsPayload.length)
    await chunkInsert("coach_assignments", assignmentsPayload);
  ...
  if (shouldImportCompetitions && compRepPayload.length)
    await chunkInsert("competition_reputation", compRepPayload);
  ...
  if (shouldImportClubs && clubRepPayload.length)
    await chunkInsert("club_reputation_season", clubRepPayload);

  // ---- Import log ----
  {
    const warnings = [...parsed.warnings];
    const recordsByScope: Record<string, number> = {
      all:
        parsed.clubCountry.length +
        parsed.clubReputation.length +
        parsed.coaches.length +
        parsed.competitionReputation.length +
        parsed.standings.length +
        parsed.continental.length +
        parsed.international.length,
      clubs: parsed.clubCountry.length + parsed.clubReputation.length,
      coaches: parsed.coaches.length,
      competitions: parsed.competitionReputation.length,
      classifications:
        parsed.standings.length + parsed.continental.length + parsed.international.length,
    };
    const { error } = await sb()
      .from("imports")
      .insert({
        ...
        warnings: {
          messages: warnings,
          records: recordsByScope[scope] ?? 0,
        },
      });
  ...
  // Re-sync the localStorage mirrors so every synchronous consumer
  // (debug pages, rankings, dashboards, entity rows) sees the values that
  // were just written to Supabase.
  if (shouldImportClubs) await hydrateClubReputationsFromDb();
  if (shouldImportCompetitions) await hydrateCompetitionReputationsFromDb();
  return {
...
export interface PlayersImportOptions {
  module?: "statistics" | "player_stats";
  filename?: string;
}
...
export async function importPlayersFile(
  ...
  const moduleName = options.module ?? "statistics";
  const seasonId = await ensureSeasonId(year);
  ...
  if (legacyClubNames.length) {
    const { data: clubRows } = await sb()
      .from("clubs")
      .select("id,name")
      .in("name", legacyClubNames);
    ((clubRows ?? []) as Array<{ id: string; name: string }>).forEach((c) =>
      legacyClubMap.set(c.name, c.id),
    );
  }
  ...
  await sb()
    ...
    .insert({
      ...
      warnings: {
        ...
        records: rows.length,
        sheets: Object.keys(parsed.bySheet),
      },
    });
