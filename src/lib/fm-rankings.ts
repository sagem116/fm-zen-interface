...
export function computeRankings(
  ...
  for (const s of input.standings) {
    const compW =
      config.competitionWeights[s.module as keyof typeof config.competitionWeights] ?? 1;
    const nlMatched =
    ...
    if (s.is_champion) {
      const bonus =
        s.module === "superleague" ? config.superleagueChampionBonus : config.nationalChampionBonus;
  ...
  const bonusAchievements: BonusAchievement[] = [];
  {
    ...
    for (const season of allSeasons) {
      ...
      for (const club of slSet) {
        ...
        const wonContTriplete = clubContWins.find((c) =>
          matchAny(config.tripleteContinentalCompetitions ?? [], c),
        );
        const wonContDobInt = clubContWins.find((c) =>
          matchAny(config.dobradinhaIntContinentalCompetitions ?? [], c),
        );
        const wonContQuad = clubContWins.find((c) =>
          matchAny(config.quadrupleContinentalCompetitions ?? [], c),
        );
        const wonCWC = clubContWins.find((c) =>
          matchAnyStrict(config.quadrupleClubWorldCupCompetitions ?? [], c),
        );
