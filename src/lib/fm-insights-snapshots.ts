...
export function buildInsights(snaps: InsightsSnapshot[]): InsightItem[] {
  ...
  if (prev.avgAge && curr.avgAge) {
    ...
    push({
      ...
      delta: d,
      pct: prev.avgAge ? (d / prev.avgAge) * 100 : 0,
      previous: prev.avgAge,
      current: curr.avgAge,
      relevance: Math.abs(d) * 50,
      category: "geral",
    });
  }
  ...
  const compKeys = new Set([
    ...Object.keys(prev.byCompetitionVp),
    ...Object.keys(curr.byCompetitionVp),
  ]);
  ...
  const ctyKeys = new Set([
    ...Object.keys(prev.byCountryPlayers),
    ...Object.keys(curr.byCountryPlayers),
  ]);
  ...
  if (
    curr.topGoalScorer &&
    (!prev.topGoalScorer || prev.topGoalScorer.name !== curr.topGoalScorer.name)
  ) {
    push({
      id: "top-gls",
      ...
      pct: 0,
      previous: prev.topGoalScorer?.name ?? null,
      current: curr.topGoalScorer.name,
      relevance: 1000,
      category: "jogador",
    });
  }
  ...
  if (curr.topClubByVp && (!prev.topClubByVp || prev.topClubByVp.name !== curr.topClubByVp.name)) {
    push({
      ...
      pct: 0,
      previous: prev.topClubByVp?.name ?? null,
      current: curr.topClubByVp.name,
      relevance: 900,
      category: "clube",
    });
  }
...
export function buildSnapshotFromRows(
  ...
  for (const r of rows) {
    if (r.nationality) {
      byCountry[r.nationality] = (byCountry[r.nationality] ?? 0) + 1;
      countries.add(r.nationality);
    }
    if (r.competition) {
      byCompVp[r.competition] = (byCompVp[r.competition] ?? 0) + (r.vp || 0);
      byCompSal[r.competition] = (byCompSal[r.competition] ?? 0) + (r.salary || 0);
      comps.add(r.competition);
    }
    if (r.club) {
      clubs.add(r.club);
      clubVp[r.club] = (clubVp[r.club] ?? 0) + (r.vp || 0);
    }
    ageSum += r.age || 0;
